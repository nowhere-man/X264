...
static void sigint_handler(int a) { b_ctrl_c = 1; }
...
const char *const x264_demuxer_names[] = {"auto", "raw", "y4m",
#if HAVE_AVS
                                          "avs",
#endif
#if HAVE_LAVF
...
const char *const x264_muxer_names[] = {"auto", "raw", "mkv", "flv",
...
static const char *const chroma_format_names[] = {
    [0] = "all", [X264_CSP_I400] = "i400", [X264_CSP_I420] = "i420", [X264_CSP_I422] = "i422", [X264_CSP_I444] = "i444"};
...
enum pulldown_type_e { X264_PULLDOWN_22 = 1, X264_PULLDOWN_32, X264_PULLDOWN_64, X264_PULLDOWN_DOUBLE, X264_PULLDOWN_TRIPLE, X264_PULLDOWN_EURO };
...
#define H2             \
    ...
    H1("      --log-level <string>    Specify the maximum level of logging [\"%s\"]\n"
       "                                  - %s\n",
       strtable_lookup(x264_log_level_names, cli_log_level - X264_LOG_NONE), stringify_names(buf, x264_log_level_names));
...
#if HAVE_FFMS
        if (b_regular && (b_auto || !strcasecmp(demuxer, "ffms")) && !ffms_input.open_file(filename, p_handle, info, opt)) {
            module = "ffms";
            b_auto = 0;
...
#endif
#if HAVE_LAVF
        if ((b_auto || !strcasecmp(demuxer, "lavf")) && !lavf_input.open_file(filename, p_handle, info, opt)) {
            module = "lavf";
            b_auto = 0;
...
#endif
#if HAVE_AVS
        if (b_regular && (b_auto || !strcasecmp(demuxer, "avs")) && !avs_input.open_file(filename, p_handle, info, opt)) {
            module = "avs";
            b_auto = 0;
            cli_input = avs_input;
        }
...
static int parse(int argc, char **argv, x264_param_t *param, cli_opt_t *opt)
{
    ...
    FAIL_IF_ERROR(!opt->hin && cli_input.open_file(input_filename, &opt->hin, &info, &input_opt), "could not open input file `%s'\n", input_filename);

    x264_reduce_fraction(&info.sar_width, &info.sar_height);
    x264_reduce_fraction(&info.fps_num, &info.fps_den);
    x264_cli_log(demuxername, X264_LOG_INFO, "%dx%d%c %u:%u @ %u/%u fps (%cfr)\n", info.width, info.height, info.interlaced ? 'i' : 'p',
                 info.sar_width, info.sar_height, info.fps_num, info.fps_den, info.vfr ? 'v' : 'c');

    FAIL_IF_ERROR(info.width <= 0 || info.height <= 0 || info.width > MAX_RESOLUTION || info.height > MAX_RESOLUTION,
    ...
    if (thread_input && info.thread_safe &&
        (b_thread_input || param->i_threads > 1 || (param->i_threads == X264_THREADS_AUTO && x264_cpu_num_processors() > 1))) {
    ...
    if (!tcfile_name && input_opt.timebase) {
        ...
        FAIL_IF_ERROR(i_user_timebase_num > UINT32_MAX || i_user_timebase_den > UINT32_MAX, "timebase you specified exceeds H.264 maximum\n");
...
static int64_t print_status(int64_t i_start, int64_t i_previous, int i_frame, int i_frame_total, int64_t i_file, x264_param_t *param, int64_t last_ts)
{
    ...
    if (i_frame_total) {
        ...
        sprintf(buf, "x264 [%.1f%%] %d/%d frames, %.2f fps, %.2f kb/s, eta %d:%02d:%02d", 100. * i_frame / i_frame_total, i_frame, i_frame_total, fps,
                bitrate, eta / 3600, (eta / 60) % 60, eta % 60);
    } else {
