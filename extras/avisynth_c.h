...
//           avs_prop_get_int, avs_prop_get_float, avs_prop_get_data, avs_prop_get_clip, avs_prop_get_frame, avs_prop_get_int_array,
//           avs_prop_get_float_array avs_prop_set_int, avs_prop_set_float, avs_prop_set_data, avs_prop_set_clip, avs_prop_set_frame,
//           avs_prop_set_int_array, avs_prop_set_float_array avs_prop_delete_key, avs_clear_map avs_new_video_frame_p, avs_new_video_frame_p_a
...
enum { AVS_SAMPLE_INT8 = 1 << 0, AVS_SAMPLE_INT16 = 1 << 1, AVS_SAMPLE_INT24 = 1 << 2, AVS_SAMPLE_INT32 = 1 << 3, AVS_SAMPLE_FLOAT = 1 << 4 };
...
enum {
    ...
    AVS_CS_PLANAR_MASK = AVS_CS_PLANAR | AVS_CS_INTERLEAVED | AVS_CS_YUV | AVS_CS_BGR | AVS_CS_YUVA | AVS_CS_SAMPLE_BITS_MASK |
...
enum {
    ...
    AVS_CS_I420 =
        AVS_CS_PLANAR | AVS_CS_YUV | AVS_CS_SAMPLE_BITS_8 | AVS_CS_UPLANEFIRST | AVS_CS_SUB_HEIGHT_2 | AVS_CS_SUB_WIDTH_2,  // YUV 4:2:0 planar
    AVS_CS_IYUV = AVS_CS_I420,
    AVS_CS_YV411 =
        AVS_CS_PLANAR | AVS_CS_YUV | AVS_CS_SAMPLE_BITS_8 | AVS_CS_VPLANEFIRST | AVS_CS_SUB_HEIGHT_1 | AVS_CS_SUB_WIDTH_4,  // YUV 4:1:1 planar
    AVS_CS_YUV9 =
        AVS_CS_PLANAR | AVS_CS_YUV | AVS_CS_SAMPLE_BITS_8 | AVS_CS_VPLANEFIRST | AVS_CS_SUB_HEIGHT_4 | AVS_CS_SUB_WIDTH_4,  // YUV 4:1:0 planar
...
enum { AVS_GETPROPERROR_UNSET = 1, AVS_GETPROPERROR_TYPE = 2, AVS_GETPROPERROR_INDEX = 4 };

// AVSPropAppendMode for avs_prop_set_...
enum { AVS_PROPAPPENDMODE_REPLACE = 0, AVS_PROPAPPENDMODE_APPEND = 1, AVS_PROPAPPENDMODE_TOUCH = 2 };
...
enum { AVS_ALLOCTYPE_NORMAL_ALLOC = 1, AVS_ALLOCTYPE_POOLED_ALLOC = 2 };
...
AVSC_INLINE int avs_is_yv24(const AVS_VideoInfo* p) { return (p->pixel_type & AVS_CS_PLANAR_MASK) == (AVS_CS_YV24 & AVS_CS_PLANAR_FILTER); }

AVSC_INLINE int avs_is_yv16(const AVS_VideoInfo* p) { return (p->pixel_type & AVS_CS_PLANAR_MASK) == (AVS_CS_YV16 & AVS_CS_PLANAR_FILTER); }

AVSC_INLINE int avs_is_yv12(const AVS_VideoInfo* p) { return (p->pixel_type & AVS_CS_PLANAR_MASK) == (AVS_CS_YV12 & AVS_CS_PLANAR_FILTER); }

AVSC_INLINE int avs_is_yv411(const AVS_VideoInfo* p) { return (p->pixel_type & AVS_CS_PLANAR_MASK) == (AVS_CS_YV411 & AVS_CS_PLANAR_FILTER); }

AVSC_INLINE int avs_is_y8(const AVS_VideoInfo* p) { return (p->pixel_type & AVS_CS_PLANAR_MASK) == (AVS_CS_Y8 & AVS_CS_PLANAR_FILTER); }
#endif
...
AVSC_INLINE int64_t avs_audio_samples_from_bytes(const AVS_VideoInfo* p, int64_t bytes) { return bytes / avs_bytes_per_audio_sample(p); }

AVSC_INLINE int64_t avs_bytes_from_audio_samples(const AVS_VideoInfo* p, int64_t samples) { return samples * avs_bytes_per_audio_sample(p); }
...
// no API for these, inline helper functions
AVSC_INLINE int avs_get_pitch(const AVS_VideoFrame* p) { return avs_get_pitch_p(p, 0); }

AVSC_INLINE int avs_get_row_size(const AVS_VideoFrame* p) { return avs_get_row_size_p(p, 0); }

AVSC_INLINE int avs_get_height(const AVS_VideoFrame* p) { return avs_get_height_p(p, 0); }

AVSC_INLINE const BYTE* avs_get_read_ptr(const AVS_VideoFrame* p) { return avs_get_read_ptr_p(p, 0); }

#ifndef AVSC_NO_DECLSPEC
// this inline function is calling an API function
AVSC_INLINE BYTE* avs_get_write_ptr(const AVS_VideoFrame* p) { return avs_get_write_ptr_p(p, 0); }
#endif
...
AVSC_API(int, avs_get_audio)(AVS_Clip*, void* buf, int64_t start, int64_t count);
// start and count are in samples

AVSC_API(int, avs_set_cache_hints)(AVS_Clip*, int cachehints, int frame_range);
...
struct AVS_FilterInfo {
    ...
    int(AVSC_CC* get_audio)(AVS_FilterInfo*, void* buf, int64_t start, int64_t count);
    int(AVSC_CC* set_cache_hints)(AVS_FilterInfo*, int cachehints, int frame_range);
...
//    clip.
AVSC_API(AVS_Clip*, avs_new_c_filter)(AVS_ScriptEnvironment* e, AVS_FilterInfo** fi, AVS_Value child, int store_child);
...
AVSC_API(int, avs_add_function)(AVS_ScriptEnvironment*, const char* name, const char* params, AVS_ApplyFunc apply, void* user_data);
...
AVSC_API(AVS_VideoFrame*, avs_new_video_frame_a)(AVS_ScriptEnvironment*, const AVS_VideoInfo* vi, int align);
...
AVSC_API(AVS_VideoFrame*, avs_subframe_planar)(AVS_ScriptEnvironment*, AVS_VideoFrame* src, int rel_offset, int new_pitch, int new_row_size,
...
AVSC_API(AVS_VideoFrame*, avs_subframe_planar_a)(AVS_ScriptEnvironment*, AVS_VideoFrame* src, int rel_offset, int new_pitch, int new_row_size,
...
AVSC_API(AVS_VideoFrame*, avs_new_video_frame_p)(AVS_ScriptEnvironment*, const AVS_VideoInfo* vi, AVS_VideoFrame* propSrc);

// with frame property source
AVSC_API(AVS_VideoFrame*, avs_new_video_frame_p_a)(AVS_ScriptEnvironment*, const AVS_VideoInfo* vi, AVS_VideoFrame* propSrc, int align);
...
AVSC_INLINE int avs_is_xx_fallback_return_false(const AVS_VideoInfo* p) { return 0; }
...
// Avisynth+ extensions do not exist in classic Avisynth so they are simulated
AVSC_INLINE int avs_component_size_fallback(const AVS_VideoInfo* p) { return 1; }

// Avisynth+ extensions do not exist in classic Avisynth so they are simulated
AVSC_INLINE int avs_bits_per_component_fallback(const AVS_VideoInfo* p) { return 8; }
