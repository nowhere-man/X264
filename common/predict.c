...
static void predict_16x16_dc_128_c(pixel *src) { PREDICT_16x16_DC(PIXEL_SPLAT_X4(1 << (BIT_DEPTH - 1))); }
...
static void predict_4x4_dc_128_c(pixel *src) { PREDICT_4x4_DC(PIXEL_SPLAT_X4(1 << (BIT_DEPTH - 1))); }
...
{
    pixel4 dc = PIXEL_SPLAT_X4((SRC(-1, 0) + SRC(-1, 1) + SRC(-1, 2) + SRC(-1, 3) + SRC(0, -1) + SRC(1, -1) + SRC(2, -1) + SRC(3, -1) + 4) >> 3);
    PREDICT_4x4_DC(dc);
}
...
void x264_predict_4x4_v_c(pixel *src) { PREDICT_4x4_DC(SRC_X4(0, -1)); }
...
static void predict_8x8_filter_c(pixel *src, pixel edge[36], int i_neighbor, int i_filters)
{
    ...
    if (i_filters & MB_LEFT) {
        ...
        PL(1) PL(2) PL(3) PL(4) PL(5) PL(6) edge[6] = edge[7] = (SRC(-1, 6) + 3 * SRC(-1, 7) + 2) >> 2;
    }

    if (i_filters & MB_TOP) {
        int have_tr = i_neighbor & MB_TOPRIGHT;
        edge[16] = ((have_lt ? SRC(-1, -1) : SRC(0, -1)) + 2 * SRC(0, -1) + SRC(1, -1) + 2) >> 2;
        PT(1) PT(2) PT(3) PT(4) PT(5) PT(6) edge[23] = (SRC(6, -1) + 2 * SRC(7, -1) + (have_tr ? SRC(8, -1) : SRC(7, -1)) + 2) >> 2;
...
#define PL(y) UNUSED int l##y = edge[14 - y];
#define PT(x) UNUSED int t##x = edge[16 + x];
#define PREDICT_8x8_LOAD_TOPLEFT int lt = edge[15];
...
static void predict_8x8_dc_128_c(pixel *src, pixel edge[36]) { PREDICT_8x8_DC(PIXEL_SPLAT_X4(1 << (BIT_DEPTH - 1))); }
static void predict_8x8_dc_left_c(pixel *src, pixel edge[36])
{
    PREDICT_8x8_LOAD_LEFT pixel4 dc = PIXEL_SPLAT_X4((l0 + l1 + l2 + l3 + l4 + l5 + l6 + l7 + 4) >> 3);
    PREDICT_8x8_DC(dc);
}
static void predict_8x8_dc_top_c(pixel *src, pixel edge[36])
{
    PREDICT_8x8_LOAD_TOP pixel4 dc = PIXEL_SPLAT_X4((t0 + t1 + t2 + t3 + t4 + t5 + t6 + t7 + 4) >> 3);
    PREDICT_8x8_DC(dc);
}
void x264_predict_8x8_dc_c(pixel *src, pixel edge[36])
{
    PREDICT_8x8_LOAD_LEFT PREDICT_8x8_LOAD_TOP pixel4 dc =
        PIXEL_SPLAT_X4((l0 + l1 + l2 + l3 + l4 + l5 + l6 + l7 + t0 + t1 + t2 + t3 + t4 + t5 + t6 + t7 + 8) >> 4);
    PREDICT_8x8_DC(dc);
}
void x264_predict_8x8_h_c(pixel *src, pixel edge[36])
{
    PREDICT_8x8_LOAD_LEFT
#define ROW(y) MPIXEL_X4(src + y * FDEC_STRIDE + 0) = MPIXEL_X4(src + y * FDEC_STRIDE + 4) = PIXEL_SPLAT_X4(l##y);
        ROW(0);
    ROW(1);
    ROW(2);
    ROW(3);
...
{
    pixel4 top[2] = {MPIXEL_X4(edge + 16), MPIXEL_X4(edge + 20)};
    for (int y = 0; y < 8; y++) {
...
{
    PREDICT_8x8_LOAD_TOP PREDICT_8x8_LOAD_LEFT PREDICT_8x8_LOAD_TOPLEFT int p1 = pack_pixel_1to2(F1(l6, l7), F2(l5, l6, l7));
