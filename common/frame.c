...
static x264_frame_t *frame_new(x264_t *h, int b_fdec)
{
    ...
    frame->i_field_cnt = -1;
    frame->i_duration = frame->i_cpb_duration = frame->i_dpb_output_delay = frame->i_cpb_delay = 0;
    frame->i_coded_fields_lookahead = frame->i_cpb_delay_lookahead = -1;
...
#define get_plane_ptr(...)                  \
    do {                                    \
        if (get_plane_ptr(__VA_ARGS__) < 0) \
            return -1;                      \
    } while (0)
...
int x264_frame_copy_picture(x264_t *h, x264_frame_t *dst, x264_picture_t *src)
{
    ...
    if (i_csp == X264_CSP_YUYV || i_csp == X264_CSP_UYVY) {
        ...
        h->mc.plane_copy_deinterleave_yuyv(dst->plane[p], dst->i_stride[p], dst->plane[p ^ 1], dst->i_stride[p ^ 1], (pixel *)src->img.plane[0],
                                           src->img.i_stride[0] / SIZEOF_PIXEL, h->param.i_width, h->param.i_height);
    } else if (i_csp == X264_CSP_V210) {
        stride[0] = src->img.i_stride[0];
        pix[0] = src->img.plane[0];

        h->mc.plane_copy_deinterleave_v210(dst->plane[0], dst->i_stride[0], dst->plane[1], dst->i_stride[1], (uint32_t *)pix[0],
                                           stride[0] / (int)sizeof(uint32_t), h->param.i_width, h->param.i_height);
        ...
        h->mc.plane_copy_deinterleave_rgb(dst->plane[1 + b], dst->i_stride[1 + b], dst->plane[0], dst->i_stride[0], dst->plane[2 - b],
                                          dst->i_stride[2 - b], (pixel *)pix[0], stride[0] / SIZEOF_PIXEL, i_csp == X264_CSP_BGRA ? 4 : 3,
                                          h->param.i_width, h->param.i_height);
    } else {
        int v_shift = CHROMA_V_SHIFT;
        get_plane_ptr(h, src, &pix[0], &stride[0], 0, 0, 0);
        h->mc.plane_copy(dst->plane[0], dst->i_stride[0], (pixel *)pix[0], stride[0] / SIZEOF_PIXEL, h->param.i_width, h->param.i_height);
        if (i_csp == X264_CSP_NV12 || i_csp == X264_CSP_NV16) {
            get_plane_ptr(h, src, &pix[1], &stride[1], 1, 0, v_shift);
            h->mc.plane_copy(dst->plane[1], dst->i_stride[1], (pixel *)pix[1], stride[1] / SIZEOF_PIXEL, h->param.i_width,
                             h->param.i_height >> v_shift);
        } else if (i_csp == X264_CSP_NV21) {
            get_plane_ptr(h, src, &pix[1], &stride[1], 1, 0, v_shift);
            h->mc.plane_copy_swap(dst->plane[1], dst->i_stride[1], (pixel *)pix[1], stride[1] / SIZEOF_PIXEL, h->param.i_width >> 1,
                                  h->param.i_height >> v_shift);
            ...
            h->mc.plane_copy_interleave(dst->plane[1], dst->i_stride[1], (pixel *)pix[1], stride[1] / SIZEOF_PIXEL, (pixel *)pix[2],
            ...
            h->mc.plane_copy(dst->plane[1], dst->i_stride[1], (pixel *)pix[1], stride[1] / SIZEOF_PIXEL, h->param.i_width, h->param.i_height);
            h->mc.plane_copy(dst->plane[2], dst->i_stride[2], (pixel *)pix[2], stride[2] / SIZEOF_PIXEL, h->param.i_width, h->param.i_height);
        }
    }
    return 0;
}
...
static ALWAYS_INLINE void plane_expand_border(pixel *pix, int i_stride, int i_width, int i_height, int i_padh, int i_padv, int b_pad_top,
                                              int b_pad_bottom, int b_chroma)
{
...
{
    int v_shift = CHROMA_V_SHIFT;
    plane_expand_border(frame->plane[plane], frame->i_stride[plane], 16 * h->mb.i_mb_width, 16 * h->mb.i_mb_height >> v_shift, PADH, PADV >> v_shift,
                        1, 1, CHROMA_H_SHIFT);
}
...
void x264_frame_expand_border_mod16(x264_t *h, x264_frame_t *frame)
{
    for (int i = 0; i < frame->i_plane; i++) {
        ...
        if (i_padx) {
            for (int y = 0; y < i_height; y++) {
                pixel_memset(&frame->plane[i][y * frame->i_stride[i] + i_width], &frame->plane[i][y * frame->i_stride[i] + i_width - 1 - h_shift],
                             i_padx >> h_shift, SIZEOF_PIXEL << h_shift);
            }
        }
        if (i_pady) {
            for (int y = i_height; y < i_height + i_pady; y++) {
                memcpy(&frame->plane[i][y * frame->i_stride[i]], &frame->plane[i][(i_height - (~y & PARAM_INTERLACED) - 1) * frame->i_stride[i]],
...
void x264_weight_scale_plane(x264_t *h, pixel *dst, intptr_t i_dst_stride, pixel *src, intptr_t i_src_stride, int i_width, int i_height,
                             x264_weight_t *w)
{
...
int x264_sync_frame_list_init(x264_sync_frame_list_t *slist, int max_size)
{
    ...
    if (x264_pthread_mutex_init(&slist->mutex, NULL) || x264_pthread_cond_init(&slist->cv_fill, NULL) ||
