...
#define PIXEL_SAD_C(name, lx, ly)                                                             \
    static int name(pixel *pix1, intptr_t i_stride_pix1, pixel *pix2, intptr_t i_stride_pix2) \
    {                                                                                         \
        int i_sum = 0;                                                                        \
        for (int y = 0; y < ly; y++) {                                                        \
            for (int x = 0; x < lx; x++) {                                                    \
...
uint64_t x264_pixel_ssd_wxh(x264_pixel_function_t *pf, pixel *pix1, intptr_t i_pix1, pixel *pix2, intptr_t i_pix2, int i_width, int i_height)
{
    uint64_t i_ssd = 0;
    int y;
...
static void pixel_ssd_nv12_core(pixel *pixuv1, intptr_t stride1, pixel *pixuv2, intptr_t stride2, int width, int height, uint64_t *ssd_u,
                                uint64_t *ssd_v)
{
    *ssd_u = 0, *ssd_v = 0;
...
void x264_pixel_ssd_nv12(x264_pixel_function_t *pf, pixel *pix1, intptr_t i_pix1, pixel *pix2, intptr_t i_pix2, int i_width, int i_height,
...
#define PIXEL_VAR2_C(name, h, shift)                                                                  \
    ...
    {                                                                                                 \
        ...
        ssd[0] = sqr_u;                                                                               \
        ssd[1] = sqr_v;                                                                               \
        return sqr_u - ((int64_t)sum_u * sum_u >> shift) + sqr_v - ((int64_t)sum_v * sum_v >> shift); \
    }
...
{
    int sum = sa8d_8x8(pix1, i_pix1, pix2, i_pix2) + sa8d_8x8(pix1 + 8, i_pix1, pix2 + 8, i_pix2) +
              sa8d_8x8(pix1 + 8 * i_pix1, i_pix1, pix2 + 8 * i_pix2, i_pix2) + sa8d_8x8(pix1 + 8 + 8 * i_pix1, i_pix1, pix2 + 8 + 8 * i_pix2, i_pix2);
    return (sum + 2) >> 2;
}
...
#define SATD_X(size, cpu)                                                                                                                         \
    static void x264_pixel_satd_x3_##size##cpu(pixel *fenc, pixel *pix0, pixel *pix1, pixel *pix2, intptr_t i_stride, int scores[3])              \
    {                                                                                                                                             \
    ...
    static void x264_pixel_satd_x4_##size##cpu(pixel *fenc, pixel *pix0, pixel *pix1, pixel *pix2, pixel *pix3, intptr_t i_stride, int scores[4]) \
    {                                                                                                                                             \
        scores[0] = x264_pixel_satd_##size##cpu(fenc, FENC_STRIDE, pix0, i_stride);                                                               \
        scores[1] = x264_pixel_satd_##size##cpu(fenc, FENC_STRIDE, pix1, i_stride);                                                               \
        scores[2] = x264_pixel_satd_##size##cpu(fenc, FENC_STRIDE, pix2, i_stride);                                                               \
...
INTRA_MBCMP_8x8(sad, , _c) INTRA_MBCMP_8x8(sa8d, , _c)
#if HIGH_BIT_DEPTH && HAVE_MMX
#define x264_predict_8x8_v_sse2 x264_predict_8x8_v_sse
    INTRA_MBCMP_8x8(sad, _mmx2, _c) INTRA_MBCMP_8x8(sa8d, _sse2, _sse2)
#endif
#if !HIGH_BIT_DEPTH && (HAVE_ARMV6 || HAVE_AARCH64)
        INTRA_MBCMP_8x8(sad, _neon, _neon) INTRA_MBCMP_8x8(sa8d, _neon, _neon)
#endif
...
#define INTRA_MBCMP(mbcmp, size, pred1, pred2, pred3, chroma, cpu, cpu2)                     \
    ...
    {                                                                                        \
        ...
        res[2] = x264_pixel_##mbcmp##_##size##cpu(fdec, FDEC_STRIDE, fenc, FENC_STRIDE);     \
            ...
            INTRA_MBCMP(sad, 4x4, v, h, dc, , , _c) INTRA_MBCMP(satd, 4x4, v, h, dc, , , _c) INTRA_MBCMP(sad, 8x8, dc, h, v, c, , _c)
                INTRA_MBCMP(satd, 8x8, dc, h, v, c, , _c) INTRA_MBCMP(sad, 8x16, dc, h, v, c, , _c) INTRA_MBCMP(satd, 8x16, dc, h, v, c, , _c)
                    INTRA_MBCMP(sad, 16x16, v, h, dc, , , _c) INTRA_MBCMP(satd, 16x16, v, h, dc, , , _c)
...
#define x264_predict_16x16_v_sse2 x264_predict_16x16_v_sse
                        INTRA_MBCMP(sad, 4x4, v, h, dc, , _mmx2, _c) INTRA_MBCMP(sad, 8x8, dc, h, v, c, _mmx2, _mmx2)
                            INTRA_MBCMP(sad, 8x16, dc, h, v, c, _mmx2, _mmx2) INTRA_MBCMP(satd, 8x16, dc, h, v, c, _mmx2, _mmx2)
                                INTRA_MBCMP(sad, 16x16, v, h, dc, , _mmx2, _mmx2) INTRA_MBCMP(sad, 8x8, dc, h, v, c, _sse2,
                                                                                              _sse2) INTRA_MBCMP(sad, 8x16, dc, h, v, c, _sse2, _sse2)
                                    INTRA_MBCMP(satd, 8x16, dc, h, v, c, _sse2, _sse2) INTRA_MBCMP(sad, 16x16, v, h, dc, , _sse2, _sse2)
                                        INTRA_MBCMP(sad, 8x8, dc, h, v, c, _ssse3, _sse2) INTRA_MBCMP(sad, 8x16, dc, h, v, c, _ssse3, _sse2)
                                            INTRA_MBCMP(satd, 8x16, dc, h, v, c, _ssse3, _sse2) INTRA_MBCMP(sad, 16x16, v, h, dc, , _ssse3, _sse2)
                                                INTRA_MBCMP(satd, 8x16, dc, h, v, c, _sse4, _sse2) INTRA_MBCMP(satd, 8x16, dc, h, v, c, _avx, _sse2)
#else
#define x264_predict_8x16c_v_mmx2 x264_predict_8x16c_v_mmx
                        INTRA_MBCMP(sad, 8x16, dc, h, v, c, _mmx2, _mmx2) INTRA_MBCMP(satd, 8x16, dc, h, v, c, _mmx2, _mmx2)
                            INTRA_MBCMP(sad, 8x16, dc, h, v, c, _sse2, _mmx2) INTRA_MBCMP(satd, 8x16, dc, h, v, c, _sse2, _mmx2)
                                INTRA_MBCMP(satd, 8x16, dc, h, v, c, _ssse3, _mmx2) INTRA_MBCMP(satd, 8x16, dc, h, v, c, _sse4, _mmx2)
                                    INTRA_MBCMP(satd, 8x16, dc, h, v, c, _avx, _mmx2) INTRA_MBCMP(satd, 8x16, dc, h, v, c, _xop, _mmx2)
#endif
#endif
#if !HIGH_BIT_DEPTH && HAVE_ARMV6
                                                    INTRA_MBCMP(sad, 4x4, v, h, dc, , _neon, _armv6) INTRA_MBCMP(satd, 4x4, v, h, dc, , _neon, _armv6)
                                                        INTRA_MBCMP(sad, 8x8, dc, h, v, c, _neon, _neon)
                                                            INTRA_MBCMP(satd, 8x8, dc, h, v, c, _neon, _neon)
                                                                INTRA_MBCMP(sad, 8x16, dc, h, v, c, _neon, _c) INTRA_MBCMP(satd, 8x16, dc, h, v, c,
                                                                                                                           _neon, _c)
                                                                    INTRA_MBCMP(sad, 16x16, v, h, dc, , _neon, _neon) INTRA_MBCMP(satd, 16x16, v, h,
                                                                                                                                  dc, , _neon, _neon)
#endif
...
static float ssim_end4(int sum0[5][4], int sum1[5][4], int width)
{
    ...
    for (int i = 0; i < width; i++) {
        ssim += ssim_end1(sum0[i][0] + sum0[i + 1][0] + sum1[i][0] + sum1[i + 1][0], sum0[i][1] + sum0[i + 1][1] + sum1[i][1] + sum1[i + 1][1],
                          sum0[i][2] + sum0[i + 1][2] + sum1[i][2] + sum1[i + 1][2], sum0[i][3] + sum0[i + 1][3] + sum1[i][3] + sum1[i + 1][3]);
    }
    return ssim;
}

float x264_pixel_ssim_wxh(x264_pixel_function_t *pf, pixel *pix1, intptr_t stride1, pixel *pix2, intptr_t stride2, int width, int height, void *buf,
                          int *cnt)
{
    int z = 0;
    float ssim = 0.0;
