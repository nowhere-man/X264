...
#define PREDICT_16x16_P(name, name2) \
    static void predict_16x16_p_##name(pixel *src) { PREDICT_16x16_P_CORE PREDICT_16x16_P_END(name2) }
...
#define PREDICT_16x16_P_ASM                                                                                           \
    asm("movdqu           %1, %%xmm1 \n"                                                                              \
        ...
        "movd         %%xmm0, %0     \n"                                                                              \
        : "=r"(H)                                                                                                     \
        : "m"(MEM_FIX(&src[-FDEC_STRIDE - 1], const pixel, 8)), "m"(MEM_FIX(&src[-FDEC_STRIDE + 8], const pixel, 8)), \
          "m"(MEM_FIX(pw_12345678, const int16_t, 8)), "m"(MEM_FIX(pw_m87654321, const int16_t, 8))                   \
        : "xmm0", "xmm1");
#else  // !HIGH_BIT_DEPTH
#define PREDICT_16x16_P_ASM                                                                                       \
    asm("movq           %1, %%mm1 \n"                                                                             \
        ...
        "movswl        %w0, %0    \n"                                                                             \
        : "=r"(H)                                                                                                 \
        : "m"(MEM_FIX(&src[-FDEC_STRIDE], const pixel, 8)), "m"(MEM_FIX(&src[-FDEC_STRIDE + 8], const pixel, 8)), \
          "m"(MEM_FIX(&src[-FDEC_STRIDE - 8], const pixel, 8)), "m"(MEM_FIX(pb_12345678, const int8_t, 8)),       \
...
#define PREDICT_16x16_P_CORE_INLINE                                                                                                \
    int H, V;                                                                                                                      \
    PREDICT_16x16_P_ASM V =                                                                                                        \
        8 * (src[15 * FDEC_STRIDE - 1] - src[-1 * FDEC_STRIDE - 1]) + 7 * (src[14 * FDEC_STRIDE - 1] - src[0 * FDEC_STRIDE - 1]) + \
        6 * (src[13 * FDEC_STRIDE - 1] - src[1 * FDEC_STRIDE - 1]) + 5 * (src[12 * FDEC_STRIDE - 1] - src[2 * FDEC_STRIDE - 1]) +  \
        4 * (src[11 * FDEC_STRIDE - 1] - src[3 * FDEC_STRIDE - 1]) + 3 * (src[10 * FDEC_STRIDE - 1] - src[4 * FDEC_STRIDE - 1]) +  \
        2 * (src[9 * FDEC_STRIDE - 1] - src[5 * FDEC_STRIDE - 1]) + 1 * (src[8 * FDEC_STRIDE - 1] - src[6 * FDEC_STRIDE - 1]);

#define PREDICT_16x16_P_INLINE(name, name2) \
    static void predict_16x16_p_##name(pixel *src) { PREDICT_16x16_P_CORE_INLINE PREDICT_16x16_P_END(name2) }
...
#define PREDICT_8x16C_P(name) \
    static void predict_8x16c_p_##name(pixel *src) { PREDICT_8x16C_P_CORE PREDICT_8x16C_P_END(name) }

#if !ARCH_X86_64 && !HIGH_BIT_DEPTH
        PREDICT_8x16C_P(mmx2)
#endif  // !ARCH_X86_64 && !HIGH_BIT_DEPTH
            PREDICT_8x16C_P(sse2) PREDICT_8x16C_P(avx) PREDICT_8x16C_P(avx2)
...
#define PREDICT_8x8C_P(name, name2) \
    static void predict_8x8c_p_##name(pixel *src) { PREDICT_8x8C_P_CORE PREDICT_8x8C_P_END(name2) }
...
#define PREDICT_8x8C_P_ASM                                                                               \
    asm("movdqa           %1, %%xmm0 \n"                                                                 \
        ...
        "movd         %%xmm0, %0     \n"                                                                 \
        : "=r"(H)                                                                                        \
        : "m"(MEM_FIX(&src[-FDEC_STRIDE], const pixel, 8)), "m"(MEM_FIX(pw_m32101234, const int16_t, 8)) \
        : "xmm0", "xmm1");
#else  // !HIGH_BIT_DEPTH
#define PREDICT_8x8C_P_ASM                                                                              \
    asm("movq           %1, %%mm0 \n"                                                                   \
        ...
        "movswl        %w0, %0    \n"                                                                   \
        : "=r"(H)                                                                                       \
        : "m"(MEM_FIX(&src[-FDEC_STRIDE], const pixel, 8)), "m"(MEM_FIX(pb_m32101234, const int8_t, 8)) \
        : "mm0", "mm1");
#endif  // HIGH_BIT_DEPTH

#define PREDICT_8x8C_P_CORE_INLINE                                                                                                                 \
    int H, V;                                                                                                                                      \
    PREDICT_8x8C_P_ASM V = 1 * (src[4 * FDEC_STRIDE - 1] - src[2 * FDEC_STRIDE - 1]) + 2 * (src[5 * FDEC_STRIDE - 1] - src[1 * FDEC_STRIDE - 1]) + \
                           3 * (src[6 * FDEC_STRIDE - 1] - src[0 * FDEC_STRIDE - 1]) + 4 * (src[7 * FDEC_STRIDE - 1] - src[-1 * FDEC_STRIDE - 1]); \
    H += -4 * src[-1 * FDEC_STRIDE - 1];

#define PREDICT_8x8C_P_INLINE(name, name2) \
    static void predict_8x8c_p_##name(pixel *src) { PREDICT_8x8C_P_CORE_INLINE PREDICT_8x8C_P_END(name2) }
#else  // !HAVE_X86_INLINE_ASM
...
#endif  // HAVE_X86_INLINE_ASM
#endif  // HIGH_BIT_DEPTH
                    PREDICT_8x8C_P_INLINE(avx, avx) PREDICT_8x8C_P_INLINE(avx2, avx2)
