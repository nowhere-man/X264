...
#define DEQUANT_SHL(x) dct[x] = (dct[x] * dequant_mf[i_mf][x]) * (1 << i_qbits)

#define DEQUANT_SHR(x) dct[x] = (dct[x] * dequant_mf[i_mf][x] + f) >> (-i_qbits)
...
static int optimize_chroma_2x2_dc(dctcoef dct[4], int dequant_mf) { return optimize_chroma_dc_internal(dct, dequant_mf, 0); }

static int optimize_chroma_2x4_dc(dctcoef dct[8], int dequant_mf) { return optimize_chroma_dc_internal(dct, dequant_mf, 1); }
...
static int decimate_score15(dctcoef *dct) { return decimate_score_internal(dct + 1, 15); }
static int decimate_score16(dctcoef *dct) { return decimate_score_internal(dct, 16); }
static int decimate_score64(dctcoef *dct) { return decimate_score_internal(dct, 64); }
...
last(4) last(8) last(15) last(16) last(64)
...
#define level_run(num)                                                        \
    ...
    level_run(4) level_run(8) level_run(15) level_run(16)
        ...
        void x264_quant_init(x264_t *h, uint32_t cpu, x264_quant_function_t *pf)
{
    ...
    pf->coeff_last[DCT_LUMA_DC] = pf->coeff_last[DCT_CHROMAU_DC] = pf->coeff_last[DCT_CHROMAV_DC] = pf->coeff_last[DCT_CHROMAU_4x4] =
        pf->coeff_last[DCT_CHROMAV_4x4] = pf->coeff_last[DCT_LUMA_4x4];
    pf->coeff_last[DCT_CHROMA_AC] = pf->coeff_last[DCT_CHROMAU_AC] = pf->coeff_last[DCT_CHROMAV_AC] = pf->coeff_last[DCT_LUMA_AC];
    ...
    pf->coeff_level_run[DCT_CHROMA_AC] = pf->coeff_level_run[DCT_CHROMAU_AC] = pf->coeff_level_run[DCT_CHROMAV_AC] = pf->coeff_level_run[DCT_LUMA_AC];
}
