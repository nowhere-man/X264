...
#define PIXEL_SAD_ALTIVEC(name, lx, ly, a, b)                                       \
    static int name(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2) \
    {                                                                               \
        ALIGNED_16(int sum);                                                        \
                                                                                    \
        LOAD_ZERO;                                                                  \
        ...
        for (int y = 0; y < ly; y++) {                                              \
            ...
            sumv = (vec_s32_t)vec_sum4s(vec_absd(pix1v, pix2v), (vec_u32_t)sumv);   \
            pix1 += i_pix1;                                                         \
            pix2 += i_pix2;                                                         \
        }                                                                           \
 ...
 **********************************************************************/
#define VEC_ABS(a) a = vec_max(a, vec_sub(zero_s16v, a));
...
static ALWAYS_INLINE vec_s32_t add_abs_4(vec_s16_t a, vec_s16_t b, vec_s16_t c, vec_s16_t d)
{
 ...
 * SATD 4x4
 **********************************************************************/
static int pixel_satd_4x4_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ...
    /* Hadamar H */
    VEC_HADAMAR(diff0v, diff1v, diff2v, diff3v, temp0v, temp1v, temp2v, temp3v);
 ...
 * SATD 4x8
 **********************************************************************/
static int pixel_satd_4x8_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ALIGNED_16(int i_satd);
...
static ALWAYS_INLINE vec_s32_t add_abs_8(vec_s16_t a, vec_s16_t b, vec_s16_t c, vec_s16_t d, vec_s16_t e, vec_s16_t f, vec_s16_t g, vec_s16_t h)
{
    vec_s16_t t0 = vec_abs(a);
    vec_s16_t t1 = vec_abs(b);
    vec_s16_t t2 = vec_abs(c);
 ...
 * SATD 8x4
 **********************************************************************/
static int pixel_satd_8x4_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ALIGNED_16(int i_satd);
    ...
    VEC_HADAMAR(diff0v, diff1v, diff2v, diff3v, temp0v, temp1v, temp2v, temp3v);
    VEC_HADAMAR(diff4v, diff5v, diff6v, diff7v, temp4v, temp5v, temp6v, temp7v);

    satdv = add_abs_8(temp0v, temp1v, temp2v, temp3v, temp4v, temp5v, temp6v, temp7v);
 ...
 * SATD 8x8
 **********************************************************************/
static int pixel_satd_8x8_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ALIGNED_16(int i_satd);
 ...
 * SATD 8x16
 **********************************************************************/
static int pixel_satd_8x16_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ALIGNED_16(int i_satd);
    ...
    satdv = vec_add(satdv, add_abs_8(temp0v, temp1v, temp2v, temp3v, temp4v, temp5v, temp6v, temp7v));
 ...
 * SATD 16x8
 **********************************************************************/
static int pixel_satd_16x8_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ALIGNED_16(int i_satd);
    ...
    VEC_HADAMAR(diffh0v, diffh1v, diffh2v, diffh3v, temp0v, temp1v, temp2v, temp3v);
    VEC_HADAMAR(diffh4v, diffh5v, diffh6v, diffh7v, temp4v, temp5v, temp6v, temp7v);
    ...
    VEC_HADAMAR(diffl0v, diffl1v, diffl2v, diffl3v, temp0v, temp1v, temp2v, temp3v);
    VEC_HADAMAR(diffl4v, diffl5v, diffl6v, diffl7v, temp4v, temp5v, temp6v, temp7v);
 ...
 * SATD 16x16
 **********************************************************************/
static int pixel_satd_16x16_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    ALIGNED_16(int i_satd);
...
static void pixel_sad_x4_16x16_altivec(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, uint8_t *pix3, intptr_t i_stride, int scores[4])
{
    ALIGNED_16(int sum0);
    ALIGNED_16(int sum1);
    ALIGNED_16(int sum2);
...
static void pixel_sad_x3_16x16_altivec(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, intptr_t i_stride, int scores[3])
{
    ALIGNED_16(int sum0);
    ALIGNED_16(int sum1);
    ALIGNED_16(int sum2);
...
static void pixel_sad_x4_16x8_altivec(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, uint8_t *pix3, intptr_t i_stride, int scores[4])
{
    ALIGNED_16(int sum0);
    ALIGNED_16(int sum1);
    ALIGNED_16(int sum2);
...
#define PIXEL_SAD_X3_ALTIVEC(name, ly)                                                                             \
    static void name(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, intptr_t i_stride, int scores[3]) \
    {                                                                                                              \
        ALIGNED_16(int sum0);                                                                                      \
        ALIGNED_16(int sum1);                                                                                      \
        ALIGNED_16(int sum2);                                                                                      \
...
static void pixel_sad_x3_16x8_altivec(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, intptr_t i_stride, int scores[3])
{
    ALIGNED_16(int sum0);
    ALIGNED_16(int sum1);
    ALIGNED_16(int sum2);
...
#define PIXEL_SAD_X4_ALTIVEC(name, ly)                                                                                            \
    static void name(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, uint8_t *pix3, intptr_t i_stride, int scores[4]) \
    {                                                                                                                             \
        ALIGNED_16(int sum0);                                                                                                     \
        ALIGNED_16(int sum1);                                                                                                     \
        ALIGNED_16(int sum2);                                                                                                     \
...
static int pixel_ssd_16x16_altivec(uint8_t *pix1, intptr_t i_stride_pix1, uint8_t *pix2, intptr_t i_stride_pix2)
{
    ALIGNED_16(int sum);
...
static int pixel_ssd_8x8_altivec(uint8_t *pix1, intptr_t i_stride_pix1, uint8_t *pix2, intptr_t i_stride_pix2)
{
    ALIGNED_16(int sum);
...
static int pixel_sa8d_8x8_core_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    int32_t i_satd = 0;
...
static int pixel_sa8d_8x8_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    int32_t i_satd;
...
static int pixel_sa8d_16x16_altivec(uint8_t *pix1, intptr_t i_pix1, uint8_t *pix2, intptr_t i_pix2)
{
    int32_t i_satd;

    i_satd = (pixel_sa8d_8x8_core_altivec(&pix1[0], i_pix1, &pix2[0], i_pix2) + pixel_sa8d_8x8_core_altivec(&pix1[8], i_pix1, &pix2[8], i_pix2) +
              pixel_sa8d_8x8_core_altivec(&pix1[8 * i_pix1], i_pix1, &pix2[8 * i_pix2], i_pix2) +
              pixel_sa8d_8x8_core_altivec(&pix1[8 * i_pix1 + 8], i_pix1, &pix2[8 * i_pix2 + 8], i_pix2) + 2) >>
             2;
    return i_satd;
}
...
static uint64_t pixel_hadamard_ac_altivec(uint8_t *pix, intptr_t stride, const vec_u8_t perm)
{
    ...
    VEC_LOAD_HIGH(pix, 2);
    VEC_LOAD_HIGH(pix, 3);
    HADAMARD4_ALTIVEC(pix16_d0, pix16_d1, pix16_d2, pix16_d3, pix16_s0, pix16_s1, pix16_s2, pix16_s3);

    VEC_LOAD_HIGH(pix, 4);
    VEC_LOAD_HIGH(pix, 5);
    VEC_LOAD_HIGH(pix, 6);
    VEC_LOAD_HIGH(pix, 7);
    HADAMARD4_ALTIVEC(pix16_d4, pix16_d5, pix16_d6, pix16_d7, pix16_s4, pix16_s5, pix16_s6, pix16_s7);

    VEC_TRANSPOSE_8(pix16_d0, pix16_d1, pix16_d2, pix16_d3, pix16_d4, pix16_d5, pix16_d6, pix16_d7, pix16_s0, pix16_s1, pix16_s2, pix16_s3, pix16_s4,
                    pix16_s5, pix16_s6, pix16_s7);
    ...
    VEC_TRANSPOSE_8(tmpi0, tmpi1, tmpi2, tmpi3, tmpi4, tmpi5, tmpi6, tmpi7, pix16_d0, pix16_d1, pix16_d2, pix16_d3, pix16_d4, pix16_d5, pix16_d6,
                    pix16_d7);

    vec_u16_t addsum04 = vec_add(VEC_ABSOLUTE(vec_add(pix16_d0, pix16_d4)), VEC_ABSOLUTE(vec_sub(pix16_d0, pix16_d4)));
    vec_u16_t addsum15 = vec_add(VEC_ABSOLUTE(vec_add(pix16_d1, pix16_d5)), VEC_ABSOLUTE(vec_sub(pix16_d1, pix16_d5)));
    vec_u16_t addsum26 = vec_add(VEC_ABSOLUTE(vec_add(pix16_d2, pix16_d6)), VEC_ABSOLUTE(vec_sub(pix16_d2, pix16_d6)));
    vec_u16_t addsum37 = vec_add(VEC_ABSOLUTE(vec_add(pix16_d3, pix16_d7)), VEC_ABSOLUTE(vec_sub(pix16_d3, pix16_d7)));
...
static const vec_u8_t hadamard_permtab[] = {CV(0x10, 0x00, 0x11, 0x01, 0x12, 0x02, 0x13, 0x03, /* pix = mod16 */
 ...
 ****************************************************************************/
static void ssim_4x4x2_core_altivec(const uint8_t *pix1, intptr_t stride1, const uint8_t *pix2, intptr_t stride2, int sums[2][4])
{
...
#define SATD_X(size)                                                                                                                         \
    static void pixel_satd_x3_##size##_altivec(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, intptr_t i_stride, int scores[3]) \
    {                                                                                                                                        \
    ...
    static void pixel_satd_x4_##size##_altivec(uint8_t *fenc, uint8_t *pix0, uint8_t *pix1, uint8_t *pix2, uint8_t *pix3, intptr_t i_stride, \
                                               int scores[4])                                                                                \
    {                                                                                                                                        \
        scores[0] = pixel_satd_##size##_altivec(fenc, FENC_STRIDE, pix0, i_stride);                                                          \
        scores[1] = pixel_satd_##size##_altivec(fenc, FENC_STRIDE, pix1, i_stride);                                                          \
...
INTRA_MBCMP_8x8(sad) INTRA_MBCMP_8x8(sa8d)
...
#define INTRA_MBCMP(mbcmp, size, pred1, pred2, pred3, chroma)                                                  \
    ...
    INTRA_MBCMP(satd, 4, v, h, dc, ) INTRA_MBCMP(sad, 8, dc, h, v, c) INTRA_MBCMP(satd, 8, dc, h, v, c) INTRA_MBCMP(sad, 16, v, h, dc, )
