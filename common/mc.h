...
#define PROPAGATE_LIST(cpu)                                                                                                                       \
    void x264_mbtree_propagate_list_internal_##cpu(int16_t (*mvs)[2], int16_t *propagate_amount, uint16_t *lowres_costs, int16_t *output,         \
                                                   int bipred_weight, int mb_y, int len);                                                         \
                                                                                                                                                  \
    static void mbtree_propagate_list_##cpu(x264_t *h, uint16_t *ref_costs, int16_t (*mvs)[2], int16_t *propagate_amount, uint16_t *lowres_costs, \
                                            int bipred_weight, int mb_y, int len, int list)                                                       \
    {                                                                                                                                             \
        int16_t *current = h->scratch_buffer2;                                                                                                    \
                                                                                                                                                  \
        x264_mbtree_propagate_list_internal_##cpu(mvs, propagate_amount, lowres_costs, current, bipred_weight, mb_y, len);                        \
                                                                                                                                                  \
...
#define PLANE_COPY_YUYV(align, cpu)                                                                                                               \
    static void plane_copy_deinterleave_yuyv_##cpu(pixel *dsta, intptr_t i_dsta, pixel *dstb, intptr_t i_dstb, pixel *src, intptr_t i_src, int w, \
                                                   int h)                                                                                         \
    {                                                                                                                                             \
        int c_w = (align >> 1) / SIZEOF_PIXEL - 1;                                                                                                \
        if (!(w & c_w))                                                                                                                           \
            ...
            if (--h > 0) {                                                                                                                        \
                if (i_src > 0) {                                                                                                                  \
                    ...
                    x264_plane_copy_deinterleave_##cpu(dsta + i_dsta, i_dsta, dstb + i_dstb, i_dstb, src + i_src, i_src, w, h);                   \
            }                                                                                                                                     \
...
#define PLANE_INTERLEAVE(cpu)                                                                                                                     \
    static void plane_copy_interleave_##cpu(pixel *dst, intptr_t i_dst, pixel *srcu, intptr_t i_srcu, pixel *srcv, intptr_t i_srcv, int w, int h) \
    {                                                                                                                                             \
...
typedef struct {
    void (*mc_luma)(pixel *dst, intptr_t i_dst, pixel **src, intptr_t i_src, int mvx, int mvy, int i_width, int i_height,
                    const x264_weight_t *weight);

    /* may round up the dimensions if they're not a power of 2 */
    pixel *(*get_ref)(pixel *dst, intptr_t *i_dst, pixel **src, intptr_t i_src, int mvx, int mvy, int i_width, int i_height,
                      const x264_weight_t *weight);
    ...
    void (*avg[12])(pixel *dst, intptr_t dst_stride, pixel *src1, intptr_t src1_stride, pixel *src2, intptr_t src2_stride, int i_weight);
    ...
    void (*plane_copy_deinterleave_yuyv)(pixel *dsta, intptr_t i_dsta, pixel *dstb, intptr_t i_dstb, pixel *src, intptr_t i_src, int w, int h);
    void (*plane_copy_deinterleave_rgb)(pixel *dsta, intptr_t i_dsta, pixel *dstb, intptr_t i_dstb, pixel *dstc, intptr_t i_dstc, pixel *src,
                                        intptr_t i_src, int pw, int w, int h);
    void (*plane_copy_deinterleave_v210)(pixel *dsty, intptr_t i_dsty, pixel *dstc, intptr_t i_dstc, uint32_t *src, intptr_t i_src, int w, int h);
    ...
    void (*frame_init_lowres_core)(pixel *src0, pixel *dst0, pixel *dsth, pixel *dstv, pixel *dstc, intptr_t src_stride, intptr_t dst_stride,
    ...
    void (*mbtree_propagate_cost)(int16_t *dst, uint16_t *propagate_in, uint16_t *intra_costs, uint16_t *inter_costs, uint16_t *inv_qscales,
                                  float *fps_factor, int len);
    void (*mbtree_propagate_list)(x264_t *h, uint16_t *ref_costs, int16_t (*mvs)[2], int16_t *propagate_amount, uint16_t *lowres_costs,
                                  int bipred_weight, int mb_y, int len, int list);
