...
void x264_plane_copy_core_neon(pixel *dst, intptr_t i_dst, pixel *src, intptr_t i_src, int w, int h);
#define x264_plane_copy_deinterleave_neon x264_template(plane_copy_deinterleave_neon)
void x264_plane_copy_deinterleave_neon(pixel *dstu, intptr_t i_dstu, pixel *dstv, intptr_t i_dstv, pixel *src, intptr_t i_src, int w, int h);
#define x264_plane_copy_deinterleave_rgb_neon x264_template(plane_copy_deinterleave_rgb_neon)
void x264_plane_copy_deinterleave_rgb_neon(pixel *dsta, intptr_t i_dsta, pixel *dstb, intptr_t i_dstb, pixel *dstc, intptr_t i_dstc, pixel *src,
...
void x264_plane_copy_interleave_core_neon(pixel *dst, intptr_t i_dst, pixel *srcu, intptr_t i_srcu, pixel *srcv, intptr_t i_srcv, int w, int h);
#define x264_plane_copy_swap_core_neon x264_template(plane_copy_swap_core_neon)
void x264_plane_copy_swap_core_neon(pixel *dst, intptr_t i_dst, pixel *src, intptr_t i_src, int w, int h);
...
#define MC_WEIGHT(func)                                                                                         \
    ...
    static weight_fn_t mc##func##_wtab_neon[6] = {                                                              \
        x264_mc_weight_w4##func##_neon,  x264_mc_weight_w4##func##_neon,  x264_mc_weight_w8##func##_neon,       \
        x264_mc_weight_w16##func##_neon, x264_mc_weight_w16##func##_neon, x264_mc_weight_w20##func##_neon,      \
    };
...
static void (*const mc_copy_wtab_neon[5])(uint8_t *, intptr_t, uint8_t *, intptr_t, int) = {
    NULL, x264_mc_copy_w4_neon, x264_mc_copy_w8_neon, NULL, x264_mc_copy_w16_neon,
};

static void mc_luma_neon(uint8_t *dst, intptr_t i_dst_stride, uint8_t *src[4], intptr_t i_src_stride, int mvx, int mvy, int i_width, int i_height,
                         const x264_weight_t *weight)
{
    ...
    if (qpel_idx & 5) /* qpel interpolation needed */
    {
        ...
        pixel_avg_wtab_neon[i_width >> 2](dst, i_dst_stride, src1, i_src_stride, src2, i_height);
        if (weight->weightfn) {
...
static uint8_t *get_ref_neon(uint8_t *dst, intptr_t *i_dst_stride, uint8_t *src[4], intptr_t i_src_stride, int mvx, int mvy, int i_width,
...
static void hpel_filter_neon(uint8_t *dsth, uint8_t *dstv, uint8_t *dstc, uint8_t *src, intptr_t stride, int width, int height, int16_t *buf)
{
