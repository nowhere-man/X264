...
static const uint8_t quant8_scan[16] = {0, 3, 4, 3, 3, 1, 5, 1, 4, 5, 2, 5, 3, 1, 5, 1};
...
int x264_cqm_init(x264_t *h)
{
    ...
    int num_8x8_lists = h->sps->i_chroma_format_idc == CHROMA_444 ? 4
                        : h->param.analyse.b_transform_8x8        ? 2
...
#define CQM_ALLOC(w, count)                                                                                                                      \
    for (int i = 0; i < count; i++) {                                                                                                            \
        ...
        for (j = 0; j < i; j++)                                                                                                                  \
            if (deadzone[j] == deadzone[i] && !memcmp(h->sps->scaling_list[i + start], h->sps->scaling_list[j + start], size * sizeof(uint8_t))) \
                break;                                                                                                                           \
        if (j < i) {                                                                                                                             \
    ...
    for (int q = 0; q < QP_MAX - QP_MAX_SPEC; q++) {
        for (int cat = 0; cat < 3 + CHROMA444; cat++) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                double start = dct8x8 ? h->unquant8_mf[CQM_8PY][QP_MAX_SPEC][i] : h->unquant4_mf[CQM_4PY][QP_MAX_SPEC][i];
...
static int cqm_parse_jmlist(x264_t *h, const char *buf, const char *name, uint8_t *cqm, const uint8_t *jvt, int length)
{
    int i;
