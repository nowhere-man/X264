...
static inline void pixel_avg(pixel *dst, intptr_t i_dst_stride, pixel *src1, intptr_t i_src1_stride, pixel *src2, intptr_t i_src2_stride, int i_width,
                             int i_height)
{
...
static inline void pixel_avg_wxh(pixel *dst, intptr_t i_dst, pixel *src1, intptr_t i_src1, pixel *src2, intptr_t i_src2, int width, int height)
{
...
static inline void pixel_avg_weight_wxh(pixel *dst, intptr_t i_dst, pixel *src1, intptr_t i_src1, pixel *src2, intptr_t i_src2, int width, int height,
                                        int i_weight1)
{
...
#define PIXEL_AVG_C(name, width, height)                                                                                                        \
    static void name(pixel *pix1, intptr_t i_stride_pix1, pixel *pix2, intptr_t i_stride_pix2, pixel *pix3, intptr_t i_stride_pix3, int weight) \
    {                                                                                                                                           \
        if (weight == 32)                                                                                                                       \
...
static void weight_cache(x264_t *h, x264_weight_t *w) { w->weightfn = h->mc.weight; }
...
static void mc_weight(pixel *dst, intptr_t i_dst_stride, pixel *src, intptr_t i_src_stride, const x264_weight_t *weight, int i_width, int i_height)
{
...
static weight_fn_t mc_weight_wtab[6] = {
    mc_weight_w2, mc_weight_w4, mc_weight_w8, mc_weight_w12, mc_weight_w16, mc_weight_w20,
};
...
static void mc_luma(pixel *dst, intptr_t i_dst_stride, pixel *src[4], intptr_t i_src_stride, int mvx, int mvy, int i_width, int i_height,
...
static pixel *get_ref(pixel *dst, intptr_t *i_dst_stride, pixel *src[4], intptr_t i_src_stride, int mvx, int mvy, int i_width, int i_height,
                      const x264_weight_t *weight)
{
    int qpel_idx = ((mvy & 3) << 2) + (mvx & 3);
    int offset = (mvy >> 2) * i_src_stride + (mvx >> 2);
...
static void mc_chroma(pixel *dstu, pixel *dstv, intptr_t i_dst_stride, pixel *src, intptr_t i_src_stride, int mvx, int mvy, int i_width, int i_height)
{
    pixel *srcp;
...
#define MC_COPY(W) \
    static void mc_copy_w##W(pixel *dst, intptr_t i_dst, pixel *src, intptr_t i_src, int i_height) { mc_copy(src, i_src, dst, i_dst, W, i_height); }
MC_COPY(16)
MC_COPY(8)
MC_COPY(4)

void x264_plane_copy_c(pixel *dst, intptr_t i_dst, pixel *src, intptr_t i_src, int w, int h)
{
    while (h--) {
...
void x264_plane_copy_swap_c(pixel *dst, intptr_t i_dst, pixel *src, intptr_t i_src, int w, int h)
{
...
void x264_plane_copy_interleave_c(pixel *dst, intptr_t i_dst, pixel *srcu, intptr_t i_srcu, pixel *srcv, intptr_t i_srcv, int w, int h)
{
...
void x264_plane_copy_deinterleave_c(pixel *dsta, intptr_t i_dsta, pixel *dstb, intptr_t i_dstb, pixel *src, intptr_t i_src, int w, int h)
{
...
static void plane_copy_deinterleave_rgb_c(pixel *dsta, intptr_t i_dsta, pixel *dstb, intptr_t i_dstb, pixel *dstc, intptr_t i_dstc, pixel *src,
...
#if WORDS_BIGENDIAN
static ALWAYS_INLINE uint32_t v210_endian_fix32(uint32_t x) { return (x << 24) + ((x << 8) & 0xff0000) + ((x >> 8) & 0xff00) + (x >> 24); }
#else
#define v210_endian_fix32(x) (x)
#endif

static void plane_copy_deinterleave_v210_c(pixel *dsty, intptr_t i_dsty, pixel *dstc, intptr_t i_dstc, uint32_t *src, intptr_t i_src, int w, int h)
{
    for (int l = 0; l < h; l++) {
        pixel *dsty0 = dsty;
        pixel *dstc0 = dstc;
...
static void prefetch_fenc_null(pixel *pix_y, intptr_t stride_y, pixel *pix_uv, intptr_t stride_uv, int mb_x) { }

static void prefetch_ref_null(pixel *pix, intptr_t stride, int parity) { }

static void memzero_aligned(void *dst, size_t n) { memset(dst, 0, n); }
...
void x264_frame_init_lowres(x264_t *h, x264_frame_t *frame)
{
    ...
    h->mc.frame_init_lowres_core(src, frame->lowres[0], frame->lowres[1], frame->lowres[2], frame->lowres[3], i_stride, frame->i_stride_lowres,
                                 frame->i_width_lowres, frame->i_lines_lowres);
...
static void frame_init_lowres_core(pixel *src0, pixel *dst0, pixel *dsth, pixel *dstv, pixel *dstc, intptr_t src_stride, intptr_t dst_stride,
...
static void mbtree_propagate_cost(int16_t *dst, uint16_t *propagate_in, uint16_t *intra_costs, uint16_t *inter_costs, uint16_t *inv_qscales,
...
static void mbtree_propagate_list(x264_t *h, uint16_t *ref_costs, int16_t (*mvs)[2], int16_t *propagate_amount, uint16_t *lowres_costs,
...
void x264_frame_filter(x264_t *h, x264_frame_t *frame, int mb_y, int b_end)
{
    ...
    for (int p = 0; p < (CHROMA444 ? 3 : 1); p++) {
        ...
        if (!b_interlaced || h->mb.b_adaptive_mbaff) {
            h->mc.hpel_filter(frame->filtered[p][1] + offs, frame->filtered[p][2] + offs, frame->filtered[p][3] + offs, frame->plane[p] + offs,
                              stride, width + 16, height - start, h->scratch_buffer);
        }
        ...
        if (b_interlaced) {
            ...
            for (int i = 0; i < 2; i++, offs += frame->i_stride[p]) {
                h->mc.hpel_filter(frame->filtered_fld[p][1] + offs, frame->filtered_fld[p][2] + offs, frame->filtered_fld[p][3] + offs,
                                  frame->plane_fld[p] + offs, stride, width + 16, height_fld - start, h->scratch_buffer);
            }
        }
    }
