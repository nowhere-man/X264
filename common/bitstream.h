...
typedef struct {
    ...
    void (*cabac_block_residual_internal)(dctcoef *l, int b_interlaced, intptr_t ctx_block_cat, x264_cabac_t *cb);
    void (*cabac_block_residual_rd_internal)(dctcoef *l, int b_interlaced, intptr_t ctx_block_cat, x264_cabac_t *cb);
    void (*cabac_block_residual_8x8_rd_internal)(dctcoef *l, int b_interlaced, intptr_t ctx_block_cat, x264_cabac_t *cb);
...
static inline int bs_pos(bs_t *s) { return (8 * (s->p - s->p_start) + (WORD_SIZE * 8) - s->i_left); }
...
static inline void bs_write_ue(bs_t *s, int val) { bs_write(s, x264_ue_size_tab[val + 1], val + 1); }
...
static ALWAYS_INLINE int bs_size_ue(unsigned int val) { return x264_ue_size_tab[val + 1]; }
