...
void x264_mb_predict_mv(x264_t *h, int i_list, int idx, int i_width, int16_t mvp[2])
{
    ...
    if ((idx & 3) >= 2 + (i_width & 1) || i_refc == -2) {
        ...
        if (SLICE_MBAFF && h->mb.cache.ref[i_list][x264_scan8[0] - 1] != -2 && MB_INTERLACED != h->mb.field[h->mb.i_mb_left_xy[0]]) {
            if (idx == 2) {
...
void x264_mb_predict_mv_pskip(x264_t *h, int16_t mv[2])
{
    ...
    if (i_refa == -2 || i_refb == -2 || !((uint32_t)i_refa | M32(mv_a)) || !((uint32_t)i_refb | M32(mv_b))) {
        M32(mv) = 0;
    } else {
...
static int mb_predict_mv_direct16x16_temporal(x264_t *h)
{
    ...
    if (PARAM_INTERLACED && h->fref[1][0]->field[mb_xy] != MB_INTERLACED) {
        if (MB_INTERLACED) {
            ...
            if ((IS_INTRA(type_col[0]) || partition_col[0] == D_16x16) && (IS_INTRA(type_col[1]) || partition_col[1] == D_16x16) &&
    ...
    for (int i8 = 0; i8 < max_i8; i8 += step) {
        int x8 = i8 & 1;
        int y8 = i8 >> 1;
        int ypart = (SLICE_MBAFF && h->fref[1][0]->field[mb_xy] != MB_INTERLACED) ? MB_INTERLACED ? y8 * 6 : 2 * (h->mb.i_mb_y & 1) + y8 : 3 * y8;
...
static ALWAYS_INLINE int mb_predict_mv_direct16x16_spatial(x264_t *h, int b_interlaced)
{
    ...
    int16_t (*l1mv[2])[2] = {(int16_t (*)[2]) & h->fref[1][0]->mv[0][i_mb_4x4], (int16_t (*)[2]) & h->fref[1][0]->mv[1][i_mb_4x4]};
    ...
    for (int i8 = 0; i8 < max_i8; i8 += step) {
        const int x8 = i8 & 1;
        const int y8 = i8 >> 1;
        int ypart = (b_interlaced && h->fref[1][0]->field[mb_xy] != MB_INTERLACED) ? MB_INTERLACED ? y8 * 6 : 2 * (h->mb.i_mb_y & 1) + y8 : 3 * y8;
...
static int mb_predict_mv_direct16x16_spatial_interlaced(x264_t *h) { return mb_predict_mv_direct16x16_spatial(h, 1); }

static int mb_predict_mv_direct16x16_spatial_progressive(x264_t *h) { return mb_predict_mv_direct16x16_spatial(h, 0); }
...
#define SET_IMVP(xy)                                               \
    ...
    if (i_ref == 0 && h->frames.b_have_lowres) {
        int idx = i_list ? h->fref[1][0]->i_frame - h->fenc->i_frame - 1 : h->fenc->i_frame - h->fref[0][0]->i_frame - 1;
