...
static int depth_filter_csp_is_supported(int csp)
{
    ...
    return csp_mask == X264_CSP_I400 || csp_mask == X264_CSP_I420 || csp_mask == X264_CSP_I422 || csp_mask == X264_CSP_I444 ||
           csp_mask == X264_CSP_YV12 || csp_mask == X264_CSP_YV16 || csp_mask == X264_CSP_YV24 || csp_mask == X264_CSP_NV12 ||
           csp_mask == X264_CSP_NV21 || csp_mask == X264_CSP_NV16 || csp_mask == X264_CSP_BGR || csp_mask == X264_CSP_RGB ||
...
#define DITHER_PLANE(pitch)                                                                                                             \
    static void dither_plane_##pitch(pixel *dst, int dst_stride, uint16_t *src, int src_stride, int width, int height, int16_t *errors) \
    {                                                                                                                                   \
...
#define CALL_DITHER_PLANE(pitch, off)                                                                                                          \
    dither_plane_##pitch(((pixel *)out->plane[i]) + off, out->stride[i] / SIZEOF_PIXEL, ((uint16_t *)img->plane[i]) + off, img->stride[i] / 2, \
                         width, height, error_buf)
