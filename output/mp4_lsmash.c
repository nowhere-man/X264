    ...
    if (p_mp4->p_root) {
        ...
        if (p_mp4->i_track) {
            ...
            if (!p_mp4->b_fragments) {
                MP4_LOG_IF_ERR(lsmash_create_explicit_timeline_map(p_mp4->p_root, p_mp4->i_track, edit), "failed to set timeline map for video.\n");
...
static int open_file(char *psz_filename, hnd_t *p_handle, cli_output_opt_t *opt)
{
    *p_handle = NULL;
    ...
    MP4_FAIL_IF_ERR_EX(!p_mp4->summary, "failed to allocate memory for summary information of video.\n");
...
static int set_param(hnd_t handle, x264_param_t *p_param)
{
    ...
    MP4_FAIL_IF_ERR(lsmash_set_movie_parameters(p_mp4->p_root, &movie_param), "failed to set movie parameters.\n");
    ...
    MP4_FAIL_IF_ERR(lsmash_set_track_parameters(p_mp4->p_root, p_mp4->i_track, &track_param), "failed to set track parameters for video.\n");
    ...
    MP4_FAIL_IF_ERR(lsmash_set_media_parameters(p_mp4->p_root, p_mp4->i_track, &media_param), "failed to set media parameters for video.\n");
...
static int write_headers(hnd_t handle, x264_nal_t *p_nal)
{
    ...
    lsmash_codec_specific_t *cs =
        lsmash_create_codec_specific_data(LSMASH_CODEC_SPECIFIC_DATA_TYPE_ISOM_VIDEO_H264, LSMASH_CODEC_SPECIFIC_FORMAT_STRUCTURED);
    ...
    MP4_FAIL_IF_ERR(!p_mp4->i_sample_entry, "failed to add sample entry for video.\n");

    /* SEI */
    p_mp4->p_sei_buffer = malloc(sei_size);
    MP4_FAIL_IF_ERR(!p_mp4->p_sei_buffer, "failed to allocate sei transition buffer.\n");
...
static int write_frame(hnd_t handle, uint8_t *p_nalu, int i_size, x264_picture_t *p_picture)
{
    ...
    MP4_FAIL_IF_ERR(!p_sample, "failed to create a video sample data.\n");
    ...
    if (p_mp4->b_dts_compress) {
        ...
        dts = p_mp4->i_numframe > p_mp4->i_delay_frames ? p_picture->i_dts * p_mp4->i_time_inc
    ...
    MP4_FAIL_IF_ERR(lsmash_append_sample(p_mp4->p_root, p_mp4->i_track, p_sample), "failed to append a video frame.\n");
