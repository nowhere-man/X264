...
#define convert_timebase_ms(timestamp, timebase) (int64_t)((timestamp) * (timebase) * 1000 + 0.5)
    ...
    if (p_flv->b_dts_compress) {
        ...
        dts = p_flv->i_framenum > p_flv->i_delay_frames ? convert_timebase_ms(p_picture->i_dts, p_flv->d_timebase)
    ...
    if (p_flv->i_framenum) {
        if (p_flv->i_prev_dts == dts) {
            x264_cli_log("flv", X264_LOG_WARNING,
        ...
        if (p_flv->i_prev_cts == cts) {
            x264_cli_log("flv", X264_LOG_WARNING,
                         "duplicate CTS %" PRId64 " generated by rounding\n"
