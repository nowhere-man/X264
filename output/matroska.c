...
static int set_param(hnd_t handle, x264_param_t *p_param)
{
    ...
    if (p_param->vui.i_sar_width && p_param->vui.i_sar_height && p_param->vui.i_sar_width != p_param->vui.i_sar_height) {
...
static int write_headers(hnd_t handle, x264_nal_t *p_nal)
{
    ...
    if (!p_mkv->width || !p_mkv->height || !p_mkv->d_width || !p_mkv->d_height) {
        return -1;
    }
    ...
    ret = mk_write_header(p_mkv->w, "x264" X264_VERSION, "V_MPEG4/ISO/AVC", avcC, avcC_len, p_mkv->frame_duration, 50000, p_mkv->width, p_mkv->height,
