...
int mk_write_header(mk_writer *w, const char *writing_app, const char *codec_id, const void *codec_private, unsigned codec_private_size,
                    int64_t default_frame_duration, int64_t timescale, unsigned width, unsigned height, unsigned d_width, unsigned d_height,
                    int display_size_units, int stereo_mode)
{
    mk_context *c, *ti, *v;
...
int mk_close(mk_writer *w, int64_t last_delta)
{
    ...
    if (w->wrote_header && x264_is_regular_file(w->fp)) {
        ...
        if (fseek(w->fp, w->duration_ptr, SEEK_SET) || mk_write_float_raw(w->root, (float)((double)total_duration / w->timescale)) < 0 ||
