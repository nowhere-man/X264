        ...
        if (p_mp4->i_track) {
            ...
            if (mdhd_duration != gf_isom_get_media_duration(p_mp4->p_file, p_mp4->i_track)) {
                ...
                uint32_t last_duration =
                    (uint32_t)(mdhd_duration > last_dts ? mdhd_duration - last_dts : (largest_pts - second_largest_pts) * p_mp4->i_time_inc);
...
static int set_param(hnd_t handle, x264_param_t *p_param)
{
    ...
    p_mp4->i_track = gf_isom_new_track(p_mp4->p_file, 0, GF_ISOM_MEDIA_VISUAL, p_mp4->i_time_res);
