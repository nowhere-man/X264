    ...
    if (fenc->lowres_mvs[0][ref0_distance][0][0] != 0x7FFF) {
        ...
        for (int y = 0; y < i_lines; y += 8, p += i_stride * 8) {
            for (int x = 0; x < i_width; x += 8, i_mb_xy++) {
                ...
                h->mc.mc_luma(p + x, i_stride, ref->lowres, i_stride, mvx + (x << 2), mvy + (y << 2), 8, 8, x264_weight_none);
            }
        }
        x264_emms();
        return dest;
...
void x264_weights_analyse(x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead)
{
    ...
    for (int plane = 0; plane <= 2 * !b_lookahead; plane++) {
        if (!plane || CHROMA_FORMAT) {
            ...
            fenc_mean[plane] =
                (float)(fenc->i_pixel_sum[plane] + zero_bias) / (fenc->i_lines[!!plane] * fenc->i_width[!!plane]) / (1 << (BIT_DEPTH - 8));
            ref_mean[plane] =
                (float)(ref->i_pixel_sum[plane] + zero_bias) / (fenc->i_lines[!!plane] * fenc->i_width[!!plane]) / (1 << (BIT_DEPTH - 8));
        } else {
            guess_scale[plane] = 1;
            fenc_mean[plane] = 0;
            ref_mean[plane] = 0;
    ...
    if (weights[1].weightfn || weights[2].weightfn) {
        ...
        while ((!both_weighted && denom == 7) ||
               (denom > 0 && !(weights[1].weightfn && (weights[1].i_scale & 1)) && !(weights[2].weightfn && (weights[2].i_scale & 1)))) {
            denom--;
            for (int i = 1; i <= 2; i++) {
    ...
    if (weights[0].weightfn && b_lookahead) {
        ...
        x264_weight_scale_plane(h, dst, ref->i_stride_lowres, src, ref->i_stride_lowres, width, height, &weights[0]);
...
static void slicetype_mb_cost(x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int b, int dist_scale_factor, int do_search[2],
                              const x264_weight_t *w, int *output_inter, int *output_intra)
{
    ...
    int16_t (*fenc_mvs[2])[2] = {b != p0 ? &fenc->lowres_mvs[0][b - p0 - 1][i_mb_xy] : NULL,
                                 b != p1 ? &fenc->lowres_mvs[1][p1 - b - 1][i_mb_xy] : NULL};
    int(*fenc_costs[2]) = {b != p0 ? &fenc->lowres_mv_costs[0][b - p0 - 1][i_mb_xy] : NULL,
                           b != p1 ? &fenc->lowres_mv_costs[1][p1 - b - 1][i_mb_xy] : NULL};
    int b_frame_score_mb = (i_mb_x > 0 && i_mb_x < h->mb.i_mb_width - 1 && i_mb_y > 0 && i_mb_y < h->mb.i_mb_height - 1) || h->mb.i_mb_width <= 2 ||
                           h->mb.i_mb_height <= 2;
...
#define LOAD_WPELS_LUMA(dst, src) (dst) = &(src)[i_pel_offset];
...
#define TRY_BIDIR(mv0, mv1, penalty)                                                                          \
    {                                                                                                         \
        ...
        if (h->param.analyse.i_subpel_refine <= 1) {                                                          \
            ...
            pixel *src1, *src2;                                                                               \
            src1 = h->mc.get_ref(pix1, &stride1, m[0].p_fref, m[0].i_stride[0], (mv0)[0], (mv0)[1], 8, 8, w); \
            src2 = h->mc.get_ref(pix2, &stride2, m[1].p_fref, m[1].i_stride[0], (mv1)[0], (mv1)[1], 8, 8, w); \
            h->mc.avg[PIXEL_8x8](pix1, 16, src1, stride1, src2, stride2, i_bipred_weight);                    \
        }                                                                                                     \
        i_cost = penalty * a->i_lambda + h->pixf.mbcmp[PIXEL_8x8](m[0].p_fenc[0], FENC_STRIDE, pix1, 16);     \
...
#define NUM_MBS \
    (h->mb.i_mb_width > 2 && h->mb.i_mb_height > 2 ? (h->mb.i_mb_width - 2) * (h->mb.i_mb_height - 2) : h->mb.i_mb_width * h->mb.i_mb_height)
...
static void slicetype_slice_cost(x264_slicetype_slice_t *s)
{
    ...
    for (h->mb.i_mb_y = start_y; h->mb.i_mb_y >= end_y; h->mb.i_mb_y--) {
        for (h->mb.i_mb_x = start_x; h->mb.i_mb_x >= end_x; h->mb.i_mb_x--) {
            slicetype_mb_cost(h, s->a, s->frames, s->p0, s->p1, s->b, s->dist_scale_factor, s->do_search, s->w, s->output_inter, s->output_intra);
        }
    }
}

/// @brief 返回frames[b]的intra_cost/inter_cost:frames[b]->i_cost_est[b-p0][p1-b]
/// @note 1. p0 == b == p1: 分析I帧的cost
/// @note 2. p0 == b < p1: 分析P帧的cost
/// @note 3. p0 < b < p1: 分析B帧的cost
static int slicetype_frame_cost(x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int b)
{
    int i_score = 0;
    int do_search[2];
    ...
    if (fenc->i_cost_est[b - p0][p1 - b] >= 0 && (!h->param.rc.i_vbv_buffer_size || fenc->i_row_satds[b - p0][p1 - b][0] != -1)) {
        ...
        if (do_search[1]) {
            fenc->lowres_mvs[1][p1 - b - 1][0][0] = 0;
        }
...
#endif
        {
...
#endif
            {
                ...
                x264_slicetype_slice_t s =
                    (x264_slicetype_slice_t){h, a, frames, p0, p1, b, dist_scale_factor, do_search, w, output_inter[0], output_intra[0]};
...
static int slicetype_frame_cost_recalculate(x264_t *h, x264_frame_t **frames, int p0, int p1, int b)
{
    ...
    for (h->mb.i_mb_y = h->mb.i_mb_height - 1; h->mb.i_mb_y >= 0; h->mb.i_mb_y--) {
        ...
        for (h->mb.i_mb_x = h->mb.i_mb_width - 1; h->mb.i_mb_x >= 0; h->mb.i_mb_x--) {
            ...
            if ((h->mb.i_mb_y > 0 && h->mb.i_mb_y < h->mb.i_mb_height - 1 && h->mb.i_mb_x > 0 && h->mb.i_mb_x < h->mb.i_mb_width - 1) ||
...
static void macroblock_tree_finish(x264_t *h, x264_frame_t *frame, float average_duration, int ref0_distance)
{
    ...
    log_trace("[lookahead][mb_tree]finish,frame->i_pts=%d,frame->i_type=%d,ref0_distance=%d,avg_duration=%f", frame->i_pts, frame->i_type,
              ref0_distance, average_duration);
...
static void macroblock_tree_propagate(x264_t *h, x264_frame_t **frames, float average_duration, int p0, int p1, int b, int referenced)
{
    ...
    log_trace("[lookahead][mb_tree]p0=%d,b=%d,p1=%d,referenced=%d,dist_scale_factor=%d,i_bipred_weight=%d,fps_factor=%f", p0, b, p1, referenced,
              dist_scale_factor, i_bipred_weight, fps_factor);
    ...
    for (h->mb.i_mb_y = 0; h->mb.i_mb_y < h->mb.i_mb_height; h->mb.i_mb_y++) {
        ...
        h->mc.mbtree_propagate_cost(buf, propagate_cost, frames[b]->i_intra_cost + mb_index, lowres_costs + mb_index,
        ...
        h->mc.mbtree_propagate_list(h, ref_costs[0], &mvs[0][mb_index], buf, &lowres_costs[mb_index], bipred_weights[0], h->mb.i_mb_y,
                                    h->mb.i_mb_width, 0);
        // 计算frames[p1]->i_propagate_cost：遗传给frames[b]的价值
        if (b != p1) {
            h->mc.mbtree_propagate_list(h, ref_costs[1], &mvs[1][mb_index], buf, &lowres_costs[mb_index], bipred_weights[1], h->mb.i_mb_y,
                                        h->mb.i_mb_width, 1);
        }
    }
...
static void vbv_lookahead(x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int num_frames, int keyframe)
{
    ...
    while (cur_nonb < num_frames) {
        ...
        if (next_nonb != cur_nonb) {
            ...
            if (prev_frame) {
                frames[next_nonb]->f_planned_cpb_duration[prev_frame_idx] =
                    (double)prev_frame->i_cpb_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
            }
            frames[next_nonb]->f_planned_cpb_duration[idx] =
                (double)frames[cur_nonb]->i_cpb_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
        ...
        for (int i = last_nonb + 1; i < cur_nonb; i++, idx++) {
            ...
            frames[next_nonb]->f_planned_cpb_duration[idx] =
                (double)frames[i]->i_cpb_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
            prev_frame = frames[i];
            prev_frame_idx = idx;
        }
        last_nonb = cur_nonb;
...
static int scenecut_internal(x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int real_scenecut)
{
    ...
    if (i_gop_size <= h->param.i_keyint_min / 4 || h->param.b_intra_refresh) {
        ...
        f_bias =
            f_thresh_min + (f_thresh_max - f_thresh_min) * (i_gop_size - h->param.i_keyint_min) / (h->param.i_keyint_max - h->param.i_keyint_min);
    }
...
void x264_slicetype_analyse(x264_t *h, int intra_minigop)
{
    ...
    if (IS_X264_TYPE_AUTO_OR_I(frames[1]->i_type) && h->param.i_scenecut_threshold &&
        scenecut(h, &a, frames, 0, 1, 1, orig_num_frames, i_max_search)) {
    ...
    if (h->param.i_bframe) {
        ...
        for (int j = 1; j < num_bframes + 1; j++) {
            if (frames[j]->i_forced_type == X264_TYPE_AUTO && IS_X264_TYPE_AUTO_OR_I(frames[j + 1]->i_forced_type) && h->param.i_scenecut_threshold &&
                scenecut(h, &a, frames, j, j + 1, 0, orig_num_frames, i_max_search)) {
                frames[j]->i_type = X264_TYPE_P;
...
void x264_slicetype_decide(x264_t *h)
{
    ...
    for (int i = 0; i < h->lookahead->next.i_size; i++) {
        ...
        h->lookahead->next.list[i]->f_duration =
            (double)h->lookahead->next.list[i]->i_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
    ...
    if (h->param.rc.b_stat_read) {
        ...
        for (int i = 0; i < h->lookahead->next.i_size; i++) {
            h->lookahead->next.list[i]->i_type = x264_ratecontrol_slice_type(h, h->lookahead->next.list[i]->i_frame);
        }
    } else if ((h->param.i_bframe && h->param.i_bframe_adaptive) || h->param.i_scenecut_threshold || h->param.rc.b_mb_tree ||
    ...
    for (bframes = 0, brefs = 0;; bframes++) {
        ...
        if (frm->i_forced_type != X264_TYPE_AUTO && frm->i_type != frm->i_forced_type &&
            ...
            x264_log(h, X264_LOG_WARNING, "forced frame type (%d) at %d was changed to frame type (%d)\n", frm->i_forced_type, frm->i_frame,
                     frm->i_type);
        }

        if (frm->i_type == X264_TYPE_BREF && h->param.i_bframe_pyramid < X264_B_PYRAMID_NORMAL && brefs == h->param.i_bframe_pyramid) {
        ...
        /* pyramid with multiple B-refs needs a big enough dpb that the preceding P-frame stays available.
           smaller dpb could be supported by smart enough use of mmco, but it's easier just to forbid it. */
        else if (frm->i_type == X264_TYPE_BREF && h->param.i_bframe_pyramid == X264_B_PYRAMID_NORMAL && brefs &&
            ...
            x264_log(h, X264_LOG_WARNING, "B-ref at frame %d incompatible with B-pyramid %s and %d reference frames\n", frm->i_frame,
                     x264_b_pyramid_names[h->param.i_bframe_pyramid], h->param.i_frame_reference);
        }
    ...
    for (int i = 0; i <= bframes; i++) {
        ...
        if (i) {
            ...
            h->lookahead->next.list[0]->f_planned_cpb_duration[i - 1] =
                (double)h->lookahead->next.list[i]->i_cpb_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
        } else {
...
int x264_rc_analyse_slice(x264_t *h)
{
    ...
    log_trace("[lookahead][last_satd]p0=%d,b=%d,p1=%d,frames[b]->i_frame=%d,frames[b]->i_type,cost=%d", p0, b, p1, frames[b]->i_frame,
              frames[b]->i_type, cost);
    return cost;
}
