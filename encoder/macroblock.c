...
static ALWAYS_INLINE void mb_encode_chroma_internal(x264_t *h, int b_inter, int i_qp, int chroma422)
{
    ...
    for (int ch = 0; ch < 2; ch++) {
        ...
        for (int i8x8 = 0; i8x8 < (chroma422 ? 2 : 1); i8x8++) {
            if (h->mb.b_trellis) {
                ...
                nz = h->quantf.quant_4x4x4(&dct4x4[i8x8 * 4], h->quant4_mf[CQM_4IC + b_inter][i_qp], h->quant4_bias[CQM_4IC + b_inter][i_qp]);
                nz_ac |= nz;
    ...
    h->mb.i_cbp_chroma +=
        (h->mb.cache.non_zero_count[x264_scan8[CHROMA_DC + 0]] | h->mb.cache.non_zero_count[x264_scan8[CHROMA_DC + 1]] | h->mb.i_cbp_chroma);
}
...
static ALWAYS_INLINE void macroblock_encode_internal(x264_t *h, int plane_count, int chroma)
{
    ...
    if (h->mb.i_type == P_SKIP) {
        ...
        if (!h->mb.b_skip_mc) {
            ...
            for (int p = 0; p < plane_count; p++) {
                h->mc.mc_luma(h->mb.pic.p_fdec[p], FDEC_STRIDE, &h->mb.pic.p_fref[0][0][p * 4], h->mb.pic.i_stride[p], mvx, mvy, 16, 16,
                              &h->sh.weight[0][p]);
            }
            ...
            if (chroma) {
                ...
                if (mvx | mvy) {
                    h->mc.mc_chroma(h->mb.pic.p_fdec[1], h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fref[0][0][4], h->mb.pic.i_stride[1], mvx,
                                    2 * mvy >> v_shift, 8, height);
                } else {
                    h->mc.load_deinterleave_chroma_fdec(h->mb.pic.p_fdec[1], h->mb.pic.p_fref[0][0][4], h->mb.pic.i_stride[1], height);
                }

                if (h->sh.weight[0][1].weightfn) {
                    h->sh.weight[0][1].weightfn[8 >> 2](h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fdec[1], FDEC_STRIDE, &h->sh.weight[0][1],
                                                        height);
                }
                if (h->sh.weight[0][2].weightfn) {
                    h->sh.weight[0][2].weightfn[8 >> 2](h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fdec[2], FDEC_STRIDE, &h->sh.weight[0][2],
                                                        height);
                }
            }
        }
            ...
            if (h->mb.b_transform_8x8) {
                ...
                for (int p = 0; p < plane_count; p++) {
                    for (int i4x4 = 0; i4x4 < 16; i4x4++) {
                        nz = h->zigzagf.sub_4x4(h->dct.luma4x4[p * 16 + i4x4], h->mb.pic.p_fenc[p] + block_idx_xy_fenc[i4x4],
    ...
    if (h->param.b_cabac) {
        cbp |= h->mb.cache.non_zero_count[x264_scan8[LUMA_DC]] << 8 | h->mb.cache.non_zero_count[x264_scan8[CHROMA_DC + 0]] << 9 |
    ...
    if (!b_force_no_skip) {
        if (h->mb.i_type == P_L0 && h->mb.i_partition == D_16x16 && !(h->mb.i_cbp_luma | h->mb.i_cbp_chroma) &&
...
static ALWAYS_INLINE int macroblock_probe_skip_internal(x264_t *h, int b_bidir, int plane_count, int chroma)
{
    ...
    for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
        ...
        if (!b_bidir) {
            ...
            /* Motion compensation */
            h->mc.mc_luma(h->mb.pic.p_fdec[p], FDEC_STRIDE, &h->mb.pic.p_fref[0][0][p * 4], h->mb.pic.i_stride[p], mvp[0], mvp[1], 16, 16,
                          &h->sh.weight[0][p]);
        }
        ...
        for (int i8x8 = 0, i_decimate_mb = 0; i8x8 < 4; i8x8++) {
            ...
            h->dctf.sub8x8_dct(dct4x4, h->mb.pic.p_fenc[p] + fenc_offset, h->mb.pic.p_fdec[p] + fdec_offset);
    ...
    if (chroma == CHROMA_420 || chroma == CHROMA_422) {
        ...
        if (!b_bidir) {
            /* Special case for mv0, which is (of course) very common in P-skip mode. */
            if (M32(mvp)) {
                h->mc.mc_chroma(h->mb.pic.p_fdec[1], h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fref[0][0][4], h->mb.pic.i_stride[1], mvp[0],
                                mvp[1] * (1 << chroma422), 8, chroma422 ? 16 : 8);
            } else {
                h->mc.load_deinterleave_chroma_fdec(h->mb.pic.p_fdec[1], h->mb.pic.p_fref[0][0][4], h->mb.pic.i_stride[1], chroma422 ? 16 : 8);
            }
        }
        ...
        for (int ch = 0; ch < 2; ch++) {
            ...
            if (!b_bidir && h->sh.weight[0][1 + ch].weightfn) {
                h->sh.weight[0][1 + ch].weightfn[8 >> 2](h->mb.pic.p_fdec[1 + ch], FDEC_STRIDE, h->mb.pic.p_fdec[1 + ch], FDEC_STRIDE,
...
void x264_noise_reduction_update(x264_t *h)
{
    ...
    for (int cat = 0; cat < 3 + CHROMA444; cat++) {
        ...
        for (int i = 0; i < size; i++) {
            h->nr_offset[cat][i] = ((uint64_t)h->param.analyse.i_noise_reduction * h->nr_count[cat] + h->nr_residual_sum[cat][i] / 2) /
    ...
    if (h->mb.b_lossless) {
        for (int p = 0; p < plane_count; p++) {
            ...
            if (h->mb.b_transform_8x8) {
                ...
                for (int i4 = i8 * 4; i4 < i8 * 4 + 4; i4++) {
                    nz = h->zigzagf.sub_4x4(h->dct.luma4x4[16 * p + i4], h->mb.pic.p_fenc[p] + block_idx_xy_fenc[i4],
        ...
        if (chroma == CHROMA_420 || chroma == CHROMA_422) {
            for (int ch = 0; ch < 2; ch++) {
                ...
                for (int i4x4 = 0; i4x4 <= chroma422; i4x4++) {
                    ...
                    nz = h->zigzagf.sub_4x4ac(h->dct.luma4x4[16 + offset + ch * 16], p_fenc + 4 * i4x4 * FENC_STRIDE, p_fdec + 4 * i4x4 * FDEC_STRIDE,
                                              &dc);
        ...
        if (h->mb.b_transform_8x8) {
            ...
            for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
                ...
                if (h->mb.b_trellis) {
                    for (int i4x4 = 0; i4x4 < 4; i4x4++) {
                        if (x264_quant_4x4_trellis(h, dct4x4[i4x4], quant_cat, i_qp, ctx_cat_plane[DCT_LUMA_4x4][p], 0, !!p,
                                                   i8 * 4 + i4x4 + p * 16)) {
