...
static inline void cabac_mb_type_intra(x264_t *h, x264_cabac_t *cb, int i_mb_type, int ctx0, int ctx1, int ctx2, int ctx3, int ctx4, int ctx5)
{
...
static void cabac_field_decoding_flag(x264_t *h, x264_cabac_t *cb)
{
    ...
    ctx +=
        (h->mb.i_mb_top_mbpair_xy >= 0 && h->mb.slice_table[h->mb.i_mb_top_mbpair_xy] == h->sh.i_first_mb && h->mb.field[h->mb.i_mb_top_mbpair_xy]);
...
static NOINLINE void cabac_ref_p(x264_t *h, x264_cabac_t *cb, int idx) { cabac_ref_internal(h, cb, 0, idx, 0); }
static NOINLINE void cabac_ref_b(x264_t *h, x264_cabac_t *cb, int i_list, int idx) { cabac_ref_internal(h, cb, i_list, idx, 1); }
...
static NOINLINE uint16_t cabac_mvd(x264_t *h, x264_cabac_t *cb, int i_list, int idx, int width)
{
    ...
    uint16_t amvd = x264_cabac_mvd_sum(h->mb.cache.mvd[i_list][x264_scan8[idx] - 1], h->mb.cache.mvd[i_list][x264_scan8[idx] - 8]);
...
static ALWAYS_INLINE void cabac_mb_header_b(x264_t *h, x264_cabac_t *cb, int i_mb_type, int chroma)
{
    ...
    if (i_mb_type == B_8x8) {
        ...
        for (int i_list = 0; i_list < 2; i_list++) {
            if (h->mb.i_partition == D_16x16) {
                if (b_list[i_list][0]) {
                    cabac_mvd(h, cb, i_list, 0, 4, 4);
                }
            } else if (h->mb.i_partition == D_16x8) {
                if (b_list[i_list][0]) {
                    cabac_mvd(h, cb, i_list, 0, 4, 2);
                }
                if (b_list[i_list][1]) {
                    cabac_mvd(h, cb, i_list, 8, 4, 2);
                }
            } else  // if( h->mb.i_partition == D_8x16 )
            {
                if (b_list[i_list][0]) {
                    cabac_mvd(h, cb, i_list, 0, 2, 4);
                }
                if (b_list[i_list][1]) {
                    cabac_mvd(h, cb, i_list, 4, 2, 4);
                }
            }
        }
    } else { /* intra */
...
static ALWAYS_INLINE void cabac_block_residual_internal(x264_t *h, x264_cabac_t *cb, int ctx_block_cat, dctcoef *l, int b_8x8, int chroma422dc)
{
    ...
    if (last != (b_8x8 ? 63 : chroma422dc ? 7 : x264_count_cat_m1[ctx_block_cat])) {
        x264_cabac_encode_decision(cb, ctx_sig + (b_8x8 ? sig_offset[last] : chroma422dc ? x264_coeff_flag_offset_chroma_422_dc[last] : last), 1);
        x264_cabac_encode_decision(cb,
                                   ctx_last + (b_8x8         ? x264_last_coeff_flag_offset_8x8[last]
                                               : chroma422dc ? x264_coeff_flag_offset_chroma_422_dc[last]
                                                             : last),
                                   1);
    }
    ...
    for (int i = last - 1; i >= 0; i--) {
        if (l[i]) {
            coeff_abs = abs(l[i]);
            x264_cabac_encode_decision(cb, ctx_sig + (b_8x8 ? sig_offset[i] : chroma422dc ? x264_coeff_flag_offset_chroma_422_dc[i] : i), 1);
            x264_cabac_encode_decision(cb,
                                       ctx_last + (b_8x8         ? x264_last_coeff_flag_offset_8x8[i]
                                                   : chroma422dc ? x264_coeff_flag_offset_chroma_422_dc[i]
                                                                 : i),
                                       0);
            ...
            x264_cabac_encode_decision(cb, ctx_sig + (b_8x8 ? sig_offset[i] : chroma422dc ? x264_coeff_flag_offset_chroma_422_dc[i] : i), 0);
        }
    }
}
...
static ALWAYS_INLINE void macroblock_write_cabac_internal(x264_t *h, x264_cabac_t *cb, int plane_count, int chroma)
{
    ...
    if (SLICE_MBAFF && (!(h->mb.i_mb_y & 1) || IS_SKIP(h->mb.type[h->mb.i_mb_xy - h->mb.i_mb_stride]))) {
...
#define RESTORE(dst, src, res) src = dst;
    ...
    if ((h->mb.i_neighbour & MB_TOP) && !h->mb.mb_transform_size[h->mb.i_mb_top_xy] && !(h->mb.cbp[h->mb.i_mb_top_xy] & 0x1000)) {          \
        ...
        MUNGE(M32(&nnzbak[2][4]), M32(&h->mb.cache.non_zero_count[x264_scan8[16 * 2] - 8]), 0x00000000U)                                    \
                ...
                MUNGE_8x8_NNZ(BACKUP)
                    ...
                    for (int p = 0; p < 3; p++) FOREACH_BIT(i, 0, h->mb.i_cbp_luma)
            ...
            for (int p = 0; p < plane_count; p++) {
                FOREACH_BIT(i8x8, 0, h->mb.i_cbp_luma) {
                    for (int i = 0; i < 4; i++) {
                        cabac_block_residual_cbf(h, cb, ctx_cat_plane[DCT_LUMA_4x4][p], i + i8x8 * 4 + p * 16, h->dct.luma4x4[i + i8x8 * 4 + p * 16],
                                                b_intra);
                    }
                }
            }
...
static void partition_size_cabac(x264_t *h, x264_cabac_t *cb, int i8, int i_pixel)
{
    ...
    if (i_mb_type == P_8x8) {
        ...
        if (x264_mb_type_list_table[i_mb_type][0][!!i8]) {
            cabac_mvd(h, cb, 0, 4 * i8, 4 >> b_8x16, 2 << b_8x16);
        }
        if (x264_mb_type_list_table[i_mb_type][1][!!i8]) {
            cabac_mvd(h, cb, 1, 4 * i8, 4 >> b_8x16, 2 << b_8x16);
        }
            ...
            if (h->mb.b_transform_8x8) {
                ...
                for (int p = 0; p < plane_count; p++) {
                    for (int i4 = 0; i4 < 4; i4++) {
                        cabac_block_residual_cbf(h, cb, ctx_cat_plane[DCT_LUMA_4x4][p], i4 + i8 * 4 + p * 16, h->dct.luma4x4[i4 + i8 * 4 + p * 16],
                                                 0);
                    }
                }
            }
