...
static const uint8_t cbp_to_golomb[2][2][48] = {
    {                                              {0, 1, 2, 5, 3, 6, 14, 10, 4, 15, 7, 11, 8, 12, 13, 9},{1, 10, 11, 6, 12, 7, 14, 2, 13, 15, 8, 3, 9, 4, 5, 0}                                           },
...
static const uint8_t subpartition_p_to_golomb[4] = {3, 1, 2, 0};

static const uint8_t subpartition_b_to_golomb[13] = {10, 4, 5, 1, 11, 6, 7, 2, 12, 8, 9, 3, 0};
...
static int cavlc_block_residual_internal(x264_t *h, int ctx_block_cat, dctcoef *l, int nC)
{
    ...
    if (ctx_block_cat == DCT_CHROMA_DC) {
        if (i_total < 8 >> CHROMA_V_SHIFT) {
            vlc_t total_zeros =
                CHROMA_FORMAT == CHROMA_420 ? x264_total_zeros_2x2_dc[i_total - 1][i_total_zero] : x264_total_zeros_2x4_dc[i_total - 1][i_total_zero];
...
#define x264_cavlc_block_residual(h, cat, idx, l)                                                                                                    \
    {                                                                                                                                                \
        int nC =                                                                                                                                     \
            cat == DCT_CHROMA_DC ? 5 - CHROMA_V_SHIFT : ct_index[x264_mb_predict_non_zero_code(h, cat == DCT_LUMA_DC ? (idx - LUMA_DC) * 16 : idx)]; \
...
static void cavlc_qp_delta(x264_t *h)
{
    ...
    if (h->mb.i_type == I_16x16 && !(h->mb.i_cbp_luma | h->mb.i_cbp_chroma) && !h->mb.cache.non_zero_count[x264_scan8[LUMA_DC]] &&
        !h->mb.cache.non_zero_count[x264_scan8[CHROMA_DC + 0]] && !h->mb.cache.non_zero_count[x264_scan8[CHROMA_DC + 1]] &&
...
static void cavlc_mb_header_i(x264_t *h, int i_mb_type, int i_mb_i_offset, int chroma)
{
    ...
    if (i_mb_type == I_16x16) {
        bs_write_ue(s, i_mb_i_offset + 1 + x264_mb_pred_mode16x16_fix[h->mb.i_intra16x16_pred_mode] + h->mb.i_cbp_chroma * 4 +
                           (h->mb.i_cbp_luma == 0 ? 0 : 12));
...
static ALWAYS_INLINE void cavlc_mb_header_p(x264_t *h, int i_mb_type, int chroma)
{
    ...
    if (i_mb_type == P_L0) {
        ...
        int b_sub_ref;
        if ((h->mb.cache.ref[0][x264_scan8[0]] | h->mb.cache.ref[0][x264_scan8[4]] | h->mb.cache.ref[0][x264_scan8[8]] |
             h->mb.cache.ref[0][x264_scan8[12]]) == 0) {
            bs_write_ue(s, 4);
            b_sub_ref = 0;
        } else {
            bs_write_ue(s, 3);
