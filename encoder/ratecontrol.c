 ...
 */
static inline float qp2qscale(float qp) { return 0.85f * powf(2.0f, (qp - (12.0f + QP_BD_OFFSET)) / 6.0f); }
static inline float qscale2qp(float qscale) { return (12.0f + QP_BD_OFFSET) + 6.0f * log2f(qscale / 0.85f); }
...
static inline double qscale2bits(ratecontrol_entry_t *rce, double qscale)
{
    ...
    return (rce->tex_bits + .1) * pow(rce->qscale / qscale, 1.1) + rce->mv_bits * pow(X264_MAX(rce->qscale, 1) / X264_MAX(qscale, 1), 0.5) +
           rce->misc_bits;
}
    ...
    if (rc->entry[frame->i_frame].kept_as_ref) {
        ...
        if (rc->mbtree.qpbuf_pos < 0) {
            do {
                ...
                if (fread(rc->mbtree.qp_buffer[rc->mbtree.qpbuf_pos], sizeof(uint16_t), rc->mbtree.src_mb_count, rc->p_mbtree_stat_file_in) !=
                    (unsigned)rc->mbtree.src_mb_count) {
                    goto fail;
                }
...
#define MAX_DURATION 0.5
            ...
            int max_cpb_output_delay =
                X264_MIN(h->param.i_keyint_max * MAX_DURATION * h->sps->vui.i_time_scale / h->sps->vui.i_num_units_in_tick, INT_MAX);
            int max_dpb_output_delay =
                h->sps->vui.i_max_dec_frame_buffering * MAX_DURATION * h->sps->vui.i_time_scale / h->sps->vui.i_num_units_in_tick;
...
#undef MAX_DURATION
        ...
        if (b_init) {
            ...
            rc->buffer_fill_final = rc->buffer_fill_final_min = rc->buffer_size * h->param.rc.f_vbv_buffer_init * h->sps->vui.i_time_scale;
            rc->b_vbv = 1;
    ...
    if (h->param.rc.b_stat_read) {
        ...
        for (int i = 0; i < rc->num_entries; i++) {
            ...
            e += sscanf(p, " in:%*d out:%*d type:%c dur:%" SCNd64 " cpbdur:%" SCNd64 " q:%f aq:%f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c",
                        &pict_type, &rce->i_duration, &rce->i_cpb_duration, &qp_rc, &qp_aq, &rce->tex_bits, &rce->mv_bits, &rce->misc_bits,
                        &rce->i_count, &rce->p_count, &rce->s_count, &rce->direct_mode);
            ...
            char *w = strchr(p, 'w');
            if (w) {
                int count = sscanf(w, "w:%hd,%hd,%hd,%hd,%hd,%hd,%hd,%hd", &rce->i_weight_denom[0], &rce->weight[0][0], &rce->weight[0][1],
                                   &rce->i_weight_denom[1], &rce->weight[1][0], &rce->weight[1][1], &rce->weight[2][0], &rce->weight[2][1]);
                if (count == 3) {
...
void x264_ratecontrol_start(x264_t *h, int i_force_qp, int overhead)
{
        ...
        if (h->sps->i_profile_idc > PROFILE_HIGH) {
            ...
            if (h->i_frame == 0) {
                ...
                rc->frame_size_maximum = 384 * BIT_DEPTH *
                                         ((double)h->fenc->i_cpb_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale) * l->mbps /
                                         mincr;
            }
        }
    }
    ...
    log_trace("[rc][start]h->i_frame=%d,i_force_qp=%d,overhead=%d,rc->buffer_rate=%f,rc->buffer_fill=%f", h->i_frame, i_force_qp, overhead,
              rc->buffer_rate, rc->buffer_fill);
    ...
    rc->qpa_rc = rc->qpa_rc_prev = rc->qpa_aq = rc->qpa_aq_prev = 0;
...
static float predict_row_size(x264_t *h, int y, float qscale)
{
    ...
    if (h->sh.i_type == SLICE_TYPE_I || qscale >= h->fref[0][0]->f_row_qscale[y]) {
        if (h->sh.i_type == SLICE_TYPE_P && h->fref[0][0]->i_type == h->fdec->i_type && h->fref[0][0]->f_row_qscale[y] > 0 &&
            h->fref[0][0]->i_row_satd[y] > 0 && (abs(h->fref[0][0]->i_row_satd[y] - h->fdec->i_row_satd[y]) < h->fdec->i_row_satd[y] / 2)) {
            float pred_t =
                h->fref[0][0]->i_row_bits[y] * h->fdec->i_row_satd[y] / h->fref[0][0]->i_row_satd[y] * h->fref[0][0]->f_row_qscale[y] / qscale;
...
// 宏块级别的码率控制，在每行宏块编码完成后被调用，动态调整后续行的QP。
int x264_ratecontrol_mb(x264_t *h, int bits)
{
    ...
    x264_emms();
    // 累加同一行每一个块的qp，qpa_rc记录码率控制层决定的QP
    ...
    log_trace("[rc][mb]upate rc->row_pred[0]");

    // row_pred[0]: 基于当前帧复杂度的预测器
    update_predictor(&rc->row_pred[0], qscale, h->fdec->i_row_satd[y], h->fdec->i_row_bits[y]);
    // row_pred[1]: 基于参考帧复杂度的预测器(仅P/B帧)
    // 只有当前行QP小于参考帧时才更新参考预测
    if (h->sh.i_type != SLICE_TYPE_I && rc->qpm < h->fref[0][0]->f_row_qp[y]) {
        log_trace("[rc][mb]upate rc->row_pred[1]");
    ...
    // 多线程下，各slice的编码进度不同步，根据当前进度，估算其他slice的最终大小
    if (h->param.b_sliced_threads) {
        ...
        float weight =
            x264_clip3f((bits_so_far_of_other_slices + rc->frame_size_estimated) / (size_of_other_slices + rc->frame_size_estimated), 0.0, 1.0);
    ...
    if (y < h->i_threadslice_end - 1) {
        ...
        // 触发qp上调循环：
        // 1. 明显超标：超出计划+容忍
        // 2. 超出计划，并且低于vbv限制之前的qp
        // 3. 可能存在缓冲区导致underflow
        while (rc->qpm < qp_max && ((b1 > rc->frame_size_planned + rc_tol) || (b1 > rc->frame_size_planned && rc->qpm < rc->qp_novbv) ||
        ...
        while (rc->qpm > qp_min && rc->qpm < prev_row_qp && (rc->qpm > h->fdec->f_row_qp[0] || rc->single_frame_vbv) && (b2 < max_frame_size) &&
...
int x264_ratecontrol_slice_type(x264_t *h, int frame_num)
{
    ...
    if (h->param.rc.b_stat_read) {
        if (frame_num >= rc->num_entries) {
            ...
            h->param.rc.i_qp_constant = (h->stat.i_frame_count[SLICE_TYPE_P] == 0)
                                            ? 24 + QP_BD_OFFSET
            ...
            rc->qp_constant[SLICE_TYPE_I] =
                x264_clip3((int)(qscale2qp(qp2qscale(h->param.rc.i_qp_constant) / h->param.rc.f_ip_factor) + 0.5), 0, QP_MAX);
            rc->qp_constant[SLICE_TYPE_B] =
                x264_clip3((int)(qscale2qp(qp2qscale(h->param.rc.i_qp_constant) * h->param.rc.f_pb_factor) + 0.5), 0, QP_MAX);
...
int x264_ratecontrol_end(x264_t *h, int bits, int *filler)
{
    ...
    if (h->param.rc.b_stat_write) {
        ...
        if (fprintf(rc->p_stat_file_out,
                    ...
                    h->fenc->i_frame, h->i_frame, c_type, h->fenc->i_duration, h->fenc->i_cpb_duration, rc->qpa_rc, h->fdec->f_qp_avg_aq,
                    h->stat.frame.i_tex_bits, h->stat.frame.i_mv_bits, h->stat.frame.i_misc_bits, h->stat.frame.i_mb_count_i,
                    h->stat.frame.i_mb_count_p, h->stat.frame.i_mb_count_skip, c_direct) < 0) {
            goto fail;
        }
        ...
        if (h->param.analyse.i_weighted_pred >= X264_WEIGHTP_SIMPLE && h->sh.weight[0][0].weightfn) {
            ...
            if (h->sh.weight[0][1].weightfn || h->sh.weight[0][2].weightfn) {
                if (fprintf(rc->p_stat_file_out, ",%d,%d,%d,%d,%d ", h->sh.weight[0][1].i_denom, h->sh.weight[0][1].i_scale,
                            h->sh.weight[0][1].i_offset, h->sh.weight[0][2].i_scale, h->sh.weight[0][2].i_offset) < 0) {
                    goto fail;
                }
    ...
    if (rc->b_abr) {
        ...
        log_trace("[rc][end]rc->qpa_rc=%f,rc->last_rceq=%f,h->fenc->f_duration=%f,rc->cplxr_sum=%f,rc->wanted_bits_window=%f", rc->qpa_rc,
                  rc->last_rceq, h->fenc->f_duration, rc->cplxr_sum, rc->wanted_bits_window);
    }
    ...
    if (h->mb.b_variable_qp) {
        if (h->sh.i_type == SLICE_TYPE_B) {
            ...
            if (h->fenc->b_last_minigop_bframe) {
                log_trace("[rc][end]update pred_b_from_p");
                update_predictor(rc->pred_b_from_p, qp2qscale(rc->qpa_rc), h->fref[1][h->i_ref[1] - 1]->i_satd, rc->bframe_bits / rc->bframes);
                rc->bframe_bits = 0;
            }
        }
    }
    ...
    *filler = update_vbv(h, bits);
    ...
    if (h->sps->vui.b_nal_hrd_parameters_present) {
        ...
        h->fenc->hrd_timing.cpb_final_arrival_time = rc->previous_cpb_final_arrival_time =
            h->fenc->hrd_timing.cpb_initial_arrival_time + (double)(bits + filler_bits) / h->sps->vui.hrd.i_bit_rate_unscaled;

        h->fenc->hrd_timing.dpb_output_time =
            (double)h->fenc->i_dpb_output_delay * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale + h->fenc->hrd_timing.cpb_removal_time;
    }
...
static double get_qscale(x264_t *h, ratecontrol_entry_t *rce, double rate_factor, int frame_num)
{
    ...
    开启MB-Tree：
        - 基于时间特征：考虑帧在时间轴上的重要性
        - 动态分析：MB-tree会分析帧间依赖关系
        - 时间权重：不同持续时间的帧有不同的感知重要性
    */
    if (h->param.rc.b_mb_tree) {
        double timescale = (double)h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
        log_trace("[rc][start][qscale]BASE_FRAME_DURATION=%f,CLIP_DURATION=%f,rce->i_duration=%d,timescale=%f,f_qcompress=%f", BASE_FRAME_DURATION,
                  CLIP_DURATION(rce->i_duration * timescale), rce->i_duration, timescale, h->param.rc.f_qcompress);

        // BASE_FRAME_DURATION = 0.04秒： 对应25fps：作为标准帧率的基准，25fps是视频编码的传统基准帧率
        /*
        帧率对视觉质量的影响
            高帧率（短持续时间）：
            - 时间掩蔽效应强 → 可以容忍更多压缩伪影
            ...
            - 静止时间长 → 伪影更易察觉
            - 应对策略：相对降低qscale
        BASE_FRAME_DURATION / CLIP_DURATION(rce->i_duration * timescale)的含义：
            > 1：当前帧比标准帧短（高帧率）
            < 1：当前帧比标准帧长（低帧率）
            = 1：标准帧率（25fps）
        */
    ...
    // 传统：基于内容复杂度：blurred_complexity是帧的纹理复杂度
    else {
        q = pow(rce->blurred_complexity, 1 - rcc->qcompress);
    }

    // 异常情况：avoid NaN's in the rc_eq
...
static float predict_size(predictor_t *p, float q, float var) { return (p->coeff * var + p->offset) / (q * p->count); }

static void update_predictor(predictor_t *p, float q, float var, float bits)
{
    log_trace("[rc][update_pred]q=%f,var=%f,bits=%f,old_coeff=%f,old_count=%f,old_decay=%f,old_offset=%f", q, var, bits, p->coeff, p->count, p->decay,
              p->offset);
    float range = 1.5;
    if (var < 10) {
        return;
    ...
    p->coeff += new_coeff;
    p->offset += new_offset;
    log_trace("[rc][update_pred]q=%f,var=%f,bits=%f,new_coeff=%f,new_count=%f,new_decay=%f,new_offset=%f", q, var, bits, p->coeff, p->count, p->decay,
              p->offset);
}

// update VBV after encoding a frame
// 输入：bits - 当前帧实际编码后的比特数
// 输出：filler - 需要添加的填充比特数
// 作用：模拟解码器的VBV缓冲区状态更新
static int update_vbv(x264_t *h, int bits)
{
    int filler = 0;
    int bitrate = h->sps->vui.hrd.i_bit_rate_unscaled;
    x264_ratecontrol_t *rcc = h->rc;             // 当前线程
    x264_ratecontrol_t *rct = h->thread[0]->rc;  // 主线程，多线程编码时需要同步信息到主线程
    int64_t buffer_size = (int64_t)h->sps->vui.hrd.i_cpb_size_unscaled * h->sps->vui.i_time_scale;

    // 用实际的QP、SATD、比特数更新对应帧类型的预测模型，让未来的比特数预测更准确
    if (rcc->last_satd >= h->mb.i_mb_count) {
        log_trace("[rc][end][vbv]update rct->pred[%d]", h->sh.i_type);
    ...
    log_trace("[rc][end][vbv][in]bits=%d,buffer_fill_final=%" PRId64 ",buffer_fill_final_min=%" PRId64 ",buffer_diff=%" PRId64, bits,
              rct->buffer_fill_final, rct->buffer_fill_final_min, buffer_diff);
    ...
    rct->buffer_fill_final += buffer_diff;
    rct->buffer_fill_final_min += buffer_diff;
    log_trace("[rc][end][vbv][out]buffer_fill_final=%" PRId64 ",buffer_fill_final_min=%" PRId64 ",buffer_diff=%" PRId64, rct->buffer_fill_final,
              rct->buffer_fill_final_min, buffer_diff);
    ...
    if (rct->buffer_fill_final > buffer_size) {
        ...
        log_trace("[rc][end][vbv]filler=%d", filler);
    }
...
static double vbv_pass1(x264_t *h, int pict_type, double q)
{
    ...
    if (rcc->b_vbv && rcc->last_satd > 0) {
        double q0 = q;
        double fenc_cpb_duration = (double)h->fenc->i_cpb_duration * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
        ...
        if (h->param.rc.i_lookahead) {
            ...
            for (int iterations = 0; iterations < 1000 && terminate != 3; iterations++) {
                ...
                log_trace("[rc][start][vbv][underflow]iterations=%d,cur_bits=%f,total_duration=%f,buffer_fill_cur:%f,target_fill:%f,q=%f", iterations,
                          cur_bits, total_duration, buffer_fill_cur, target_fill, q);
                // 保持缓冲区至少50%填充
                ...
                // 理想目标：当前填充度 - 50%的最大填充速率
                // 上限约束：不超过缓冲区大小的80%
                // 下限约束：不低于缓冲区大小的80%
                target_fill = x264_clip3f(rcc->buffer_fill - total_duration * rcc->vbv_max_rate * 0.5, rcc->buffer_size * 0.8, rcc->buffer_size);
                log_trace("[rc][start][vbv][overflow]iterations=%d,cur_bits=%f,total_duration=%f,buffer_fill_cur:%f,target_fill:%f,q=%f", iterations,
                          cur_bits, total_duration, buffer_fill_cur, target_fill, q);
        ...
        /* Fallback to old purely-reactive algorithm: no lookahead. */
            ...
            if ((pict_type == SLICE_TYPE_P || (pict_type == SLICE_TYPE_I && rcc->last_non_b_pict_type == SLICE_TYPE_I)) &&
        ...
        // P帧的QP决策不仅影响P帧本身，还会影响依赖它的所有B帧，因此需要考虑整个mini-GOP的缓冲区消耗。
        // 如果在前面的vbv流程中：P帧QP ↓ → P帧变大 → 后续B帧QP也↓ → B帧也变大 → 整个mini-GOP消耗增加
        if (h->sh.i_type == SLICE_TYPE_P && !rcc->single_frame_vbv) {
            ...
            // 如果B帧总消耗超过其时间窗口内的最大可用比特，则忽略B帧影响
            if (bbits * nb > bframe_cpb_duration * rcc->vbv_max_rate) {
                nb = 0;
                bframe_cpb_duration = 0;
            }
            // Mini-GOP总消耗计算
            pbbits += nb * bbits;                                            // P帧 + 所有B帧的总大小
            minigop_cpb_duration = bframe_cpb_duration + fenc_cpb_duration;  // mini-GOP总时长
            // space：当前填充 + 时间窗口内最大输入 - 缓冲区大小
            space = rcc->buffer_fill + minigop_cpb_duration * rcc->vbv_max_rate - rcc->buffer_size;

            // mini-GOP消耗不足以填满过剩的空间，需要降低qscale
            if (pbbits < space) {
                // pbbits / space - 按比例减少QP
                // bits / (0.5 * rcc->buffer_size) - 基于P帧大小的保守调整
                // 取较大值：避免过度降低QP
                q *= X264_MAX(pbbits / space, bits / (0.5 * rcc->buffer_size));
            }
            q = X264_MAX(q0 / 2, q);
            log_trace("[rc][start][vbv][pict_type]nb=%d,bits=%f,pbbits=%f,bbits=%f,space=%f,bframe_cpb_duration=%f,fenc_cpb_duration=%f,q=%f", nb,
                      bits, pbbits, bbits, space, bframe_cpb_duration, fenc_cpb_duration, q);
        }
...
static float rate_estimate_qscale(x264_t *h)
{
    ...
    int64_t total_bits =
        8 * (h->stat.i_frame_size[SLICE_TYPE_I] + h->stat.i_frame_size[SLICE_TYPE_P] + h->stat.i_frame_size[SLICE_TYPE_B]) - rcc->filler_bits_sum;
    ...
    if (pict_type == SLICE_TYPE_B) {
        ...
        } else /* 1pass ABR */
        {
            ...
            log_trace("[rc][start]pict_type=%d,total_bits=%lld,rcc->last_satd=%d,rcc->short_term_cplxsum=%f,rcc->short_term_cplxcount=%f", pict_type,
                      total_bits, rcc->last_satd, rcc->short_term_cplxsum, rcc->short_term_cplxcount);
            ...
            if (h->param.rc.i_rc_method == X264_RC_CRF) {
                ...
                if (!rcc->b_vbv_min_rate && rcc->last_satd) {
                    ...
                    if (h->param.b_vfr_input && i_frame_done > 0) {
                        time_done =
                            ((double)(h->fenc->i_reordered_pts - h->i_reordered_pts_delay)) * h->param.i_timebase_num / h->param.i_timebase_den;
                    }
                    ...
                    log_trace("[rc][start]iframe_done=%d,time_done=%f,wanted_bits=%f,abr_buffer=%f,overflow=%f,qscale=%f", i_frame_done, time_done,
                              wanted_bits, abr_buffer, overflow, q);
                }
            }
            ...
            if (pict_type == SLICE_TYPE_I &&
                ...
                log_trace("[rc][start]fenc->i_frame=%d,rcc->accum_p_qp=%f,rcc->accum_p_norm=%f,qscale=%f", h->fenc->i_frame, rcc->accum_p_qp,
                          rcc->accum_p_norm, q);
            } else if (h->i_frame > 0) {
                // 防止质量剧烈波动
                if (h->param.rc.i_rc_method != X264_RC_CRF) {
                    ...
                    if (overflow > 1.1 && h->i_frame > 3) {
                        lmax *= rcc->lstep;  // 溢出时放宽上限
                    } else if (overflow < 0.9) {
                        lmin /= rcc->lstep;  // 欠载时收紧下限
                    }
        ...
        if (rcc->b_vbv) {
            ...
            log_trace("[rc][start][vbv]frame_size_maximum=%f,rcc->frame_size_planned=%f,q=%f", frame_size_maximum, rcc->frame_size_planned, q);
        }
...
void x264_threads_merge_ratecontrol(x264_t *h)
{
    ...
    for (int i = 0; i < h->param.i_threads; i++) {
        ...
        if (h->param.rc.i_vbv_buffer_size) {
            ...
            log_trace("[threaded_slices_write]update rc->pred[%d]", h->sh.i_type + (i + 1) * 5);
...
static int init_pass2(x264_t *h)
{
    ...
    for (double step = 1E4 * step_mult; step > 1E-7 * step_mult; step *= 0.5) {
        ...
        rcc->last_qscale_for[0] = rcc->last_qscale_for[1] = rcc->last_qscale_for[2] = pow(base_cplx, 1 - rcc->qcompress) / rate_factor;
    ...
    if (fabs(expected_bits / all_available_bits - 1.0) > 0.01) {
        ...
        x264_log(h, X264_LOG_WARNING, "target: %.2f kbit/s, expected: %.2f kbit/s, avg QP: %.4f\n", (float)h->param.rc.i_bitrate,
                 expected_bits * rcc->fps / (rcc->num_entries * 1000.), avgq);
