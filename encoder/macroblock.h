...
#define x264_macroblock_probe_pskip(h) x264_macroblock_probe_skip(h, 0)
#define x264_macroblock_probe_bskip(h) x264_macroblock_probe_skip(h, 1)
...
#define FOREACH_BIT(idx, start, mask) \
    for (int idx = start, msk = mask, skip; msk && (skip = x264_ctz_4bit(msk), idx += skip, msk >>= skip + 1, 1); idx++)
