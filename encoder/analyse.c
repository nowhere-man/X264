...
static const uint8_t i_mb_b_cost_table[X264_MBTYPE_MAX] = {9, 9, 9, 9, 0, 0, 0, 1, 3, 7, 7, 7, 3, 7, 7, 7, 5, 9, 0};
static const uint8_t i_mb_b16x8_cost_table[17] = {0, 0, 0, 0, 0, 0, 0, 0, 5, 7, 7, 7, 5, 7, 9, 9, 9};
static const uint8_t i_sub_mb_b_cost_table[13] = {7, 5, 5, 3, 7, 5, 7, 3, 7, 7, 7, 5, 1};
static const uint8_t i_sub_mb_p_cost_table[4] = {5, 3, 3, 1};
...
static int init_costs(x264_t *h, float *logs, int qp)
{
    ...
    for (int i = 0; i <= 2 * 4 * mv_range; i++) {
        h->cost_mv[qp][-i] = h->cost_mv[qp][i] = X264_MIN((int)(lambda * logs[i] + .5f), UINT16_MAX);
    }
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 33; j++) {
            ...
            if (height) {
                for (int k = j; k < h->i_ref[0]; k++) {
                    if (h->sh.weight[k][0].weightfn) {
                        ...
                        x264_weight_scale_plane(h, dst + offset, frame->i_stride[0], src + offset, frame->i_stride[0], width, height,
                                                &h->sh.weight[k][0]);
                    }
                }
            }
...
static void mb_analyse_init(x264_t *h, x264_mb_analysis_t *a, int qp)
{
    ...
    /* I: Intra part */
    a->i_satd_i16x16 = a->i_satd_i8x8 = a->i_satd_i4x4 = COST_MAX;
    ...
    if (h->sh.i_type != SLICE_TYPE_I) {
        ...
        a->l0.me16x16.cost = a->l0.i_rd16x16 = a->l0.i_cost8x8 = a->l0.i_cost16x8 = a->l0.i_cost8x16 = COST_MAX;
        if (h->sh.i_type == SLICE_TYPE_B) {
            a->l1.me16x16.cost = a->l1.i_rd16x16 = a->l1.i_cost8x8 = a->i_cost8x8direct[0] = a->i_cost8x8direct[1] = a->i_cost8x8direct[2] =
                a->i_cost8x8direct[3] = a->l1.i_cost16x8 = a->l1.i_cost8x16 = a->i_rd16x16bi = a->i_rd16x16direct = a->i_rd8x8bi = a->i_rd16x8bi =
                    a->i_rd8x16bi = a->i_cost16x16bi = a->i_cost16x16direct = a->i_cost8x8bi = a->i_cost16x8bi = a->i_cost8x16bi = COST_MAX;
        ...
        if (a->b_early_terminate && h->mb.i_mb_xy - h->sh.i_first_mb > 4) {
            if (IS_INTRA(h->mb.i_mb_type_left[0]) || IS_INTRA(h->mb.i_mb_type_top) || IS_INTRA(h->mb.i_mb_type_topleft) ||
                IS_INTRA(h->mb.i_mb_type_topright) || (h->sh.i_type == SLICE_TYPE_P && IS_INTRA(h->fref[0][0]->mb_type[h->mb.i_mb_xy])) ||
                (h->mb.i_mb_xy - h->sh.i_first_mb <
                 3 * (h->stat.frame.i_mb_count[I_4x4] + h->stat.frame.i_mb_count[I_8x8] + h->stat.frame.i_mb_count[I_16x16] +
        ...
        h->mb.b_skip_mc = 0;
        if (h->param.b_intra_refresh && h->sh.i_type == SLICE_TYPE_P && h->mb.i_mb_x >= h->fdec->i_pir_start_col &&
...
static void mb_analyse_intra(x264_t *h, x264_mb_analysis_t *a, int i_satd_inter)
{
    ...
    static const int8_t intra_analysis_shortcut[2][2][2][5] = {
        {{{I_PRED_4x4_HU, -1, -1, -1, -1}, {I_PRED_4x4_DDL, I_PRED_4x4_VL, -1, -1, -1}},
         {{I_PRED_4x4_DDR, I_PRED_4x4_HD, I_PRED_4x4_HU, -1, -1}, {I_PRED_4x4_DDL, I_PRED_4x4_DDR, I_PRED_4x4_VR, I_PRED_4x4_VL, -1}}},
        {                       {{I_PRED_4x4_HU, -1, -1, -1, -1}, {-1, -1, -1, -1, -1}},
         {{I_PRED_4x4_DDR, I_PRED_4x4_HD, I_PRED_4x4_HU, -1, -1}, {I_PRED_4x4_DDR, I_PRED_4x4_VR, -1, -1, -1}}                       },
    };
...
static void intra_rd_refine(x264_t *h, x264_mb_analysis_t *a)
{
    ...
    if (h->mb.i_type == I_4x4) {
        ...
        for (int idx = 0; idx < 16; idx++) {
            pixel *dst[3] = {h->mb.pic.p_fdec[0] + block_idx_xy_fdec[idx], CHROMA_FORMAT ? h->mb.pic.p_fdec[1] + block_idx_xy_fdec[idx] : NULL,
...
#define REF_COST(list, ref) (a->p_cost_ref[list][ref])
/*
Output：
...
static void mb_analyse_inter_p16x16(x264_t *h, x264_mb_analysis_t *a)
{
    ...
    for (int i_ref = 0; i_ref < h->mb.pic.i_fref[0]; i_ref++) {
        ...
        // P-SKIP模式检查
        if (i_ref == 0 && a->b_try_skip && m.cost - m.cost_mv < 300 * a->i_lambda &&
            abs(m.mv[0] - h->mb.cache.pskip_mv[0]) + abs(m.mv[1] - h->mb.cache.pskip_mv[1]) <= 1 && x264_macroblock_probe_pskip(h)) {
            h->mb.i_type = P_SKIP;
            analyse_update_cache(h, a);
...
#undef CHECK_NEIGHBOUR
    ...
    if (!h->param.b_cabac && !(a->l0.me8x8[0].i_ref | a->l0.me8x8[1].i_ref | a->l0.me8x8[2].i_ref | a->l0.me8x8[3].i_ref)) {
...
static ALWAYS_INLINE int mb_analyse_inter_p4x4_chroma_internal(x264_t *h, x264_mb_analysis_t *a, pixel **p_fref, int i8x8, int size, int chroma)
{
...
#define CHROMA4x4MC(width, height, me, x, y)                                                                                                   \
    if (chroma == CHROMA_444) {                                                                                                                \
        ...
        h->mc.mc_luma(&pix1[2 * x + 2 * y * 16], 16, &h->mb.pic.p_fref[0][i_ref][4], i_stride, mvx, mvy, 2 * width, 2 * height,                \
                      &h->sh.weight[i_ref][1]);                                                                                                \
        h->mc.mc_luma(&pix2[2 * x + 2 * y * 16], 16, &h->mb.pic.p_fref[0][i_ref][8], i_stride, mvx, mvy, 2 * width, 2 * height,                \
                      &h->sh.weight[i_ref][2]);                                                                                                \
    } else {                                                                                                                                   \
        int offset = x + (2 >> chroma_v_shift) * 16 * y;                                                                                       \
        int chroma_height = (2 >> chroma_v_shift) * height;                                                                                    \
        h->mc.mc_chroma(&pix1[offset], &pix2[offset], 16, &p_fref[4][or + 2 * x + (2 >> chroma_v_shift) * y * i_stride], i_stride, (me).mv[0], \
                        (2 >> chroma_v_shift) * ((me).mv[1] + mvy_offset), width, chroma_height);                                              \
...
static void mb_analyse_inter_p4x4(x264_t *h, x264_mb_analysis_t *a, int i8x8)
{
    ...
    a->l0.i_cost4x4[i8x8] = a->l0.me4x4[i8x8][0].cost + a->l0.me4x4[i8x8][1].cost + a->l0.me4x4[i8x8][2].cost + a->l0.me4x4[i8x8][3].cost +
...
static void mb_analyse_inter_p8x4(x264_t *h, x264_mb_analysis_t *a, int i8x8)
{
    ...
    a->l0.i_cost8x4[i8x8] =
        a->l0.me8x4[i8x8][0].cost + a->l0.me8x4[i8x8][1].cost + REF_COST(0, i_ref) + a->i_lambda * i_sub_mb_p_cost_table[D_L0_8x4];
    if (h->mb.b_chroma_me && !CHROMA444) {
...
static void mb_analyse_inter_p4x8(x264_t *h, x264_mb_analysis_t *a, int i8x8)
{
    ...
    a->l0.i_cost4x8[i8x8] =
        a->l0.me4x8[i8x8][0].cost + a->l0.me4x8[i8x8][1].cost + REF_COST(0, i_ref) + a->i_lambda * i_sub_mb_p_cost_table[D_L0_4x8];
    if (h->mb.b_chroma_me && !CHROMA444) {
...
#define COST_BI_CHROMA(m0, m1, width, height)                                                                                                  \
    {                                                                                                                                          \
        if (CHROMA444) {                                                                                                                       \
            h->mc.mc_luma(pix[0], 16, &m0.p_fref[4], m0.i_stride[1], m0.mv[0], m0.mv[1], width, height, x264_weight_none);                     \
            h->mc.mc_luma(pix[1], 16, &m0.p_fref[8], m0.i_stride[2], m0.mv[0], m0.mv[1], width, height, x264_weight_none);                     \
            h->mc.mc_luma(pix[2], 16, &m1.p_fref[4], m1.i_stride[1], m1.mv[0], m1.mv[1], width, height, x264_weight_none);                     \
            h->mc.mc_luma(pix[3], 16, &m1.p_fref[8], m1.i_stride[2], m1.mv[0], m1.mv[1], width, height, x264_weight_none);                     \
        } else {                                                                                                                               \
            ...
            h->mc.mc_chroma(pix[0], pix[1], 16, m0.p_fref[4], m0.i_stride[1], m0.mv[0], 2 * (m0.mv[1] + l0_mvy_offset) >> v_shift, width >> 1, \
                            height >> v_shift);                                                                                                \
            h->mc.mc_chroma(pix[2], pix[3], 16, m1.p_fref[4], m1.i_stride[1], m1.mv[0], 2 * (m1.mv[1] + l1_mvy_offset) >> v_shift, width >> 1, \
                            height >> v_shift);                                                                                                \
        }                                                                                                                                      \
        ...
        i_chroma_cost =                                                                                                                        \
            h->pixf.mbcmp[chromapix](m0.p_fenc[1], FENC_STRIDE, bi[0], 16) + h->pixf.mbcmp[chromapix](m0.p_fenc[2], FENC_STRIDE, bi[1], 16);   \
    }
    ...
    if (h->param.analyse.inter & X264_ANALYSE_BSUB16x16) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (h->mb.b_chroma_me) {
                ...
                a->i_cost8x8direct[i] +=
                    h->pixf.mbcmp[chromapix](&h->mb.pic.p_fenc[1][fenc_offset], FENC_STRIDE, &h->mb.pic.p_fdec[1][fdec_offset], FDEC_STRIDE) +
                    h->pixf.mbcmp[chromapix](&h->mb.pic.p_fenc[2][fenc_offset], FENC_STRIDE, &h->mb.pic.p_fdec[2][fdec_offset], FDEC_STRIDE);
            }
...
static void mb_analyse_inter_b16x16(x264_t *h, x264_mb_analysis_t *a)
{
    ...
    for (int l = 1; l >= 0;) {
        ...
        for (i_ref = (list1_skipped && l == 1) ? 1 : 0; i_ref < h->mb.pic.i_fref[l]; i_ref++) {
            ...
            /* Fast skip detection. */
            if (i_ref == 0 && try_skip) {
                if (abs(lX->me16x16.mv[0] - h->mb.cache.direct_mv[l][0][0]) + abs(lX->me16x16.mv[1] - h->mb.cache.direct_mv[l][0][1]) > 1) {
                    try_skip = 0;
                } else if (!l) {
    ...
    src0 = h->mc.get_ref(pix0, &stride0, h->mb.pic.p_fref[0][a->l0.bi16x16.i_ref], h->mb.pic.i_stride[0], a->l0.bi16x16.mv[0], a->l0.bi16x16.mv[1],
                         16, 16, x264_weight_none);
    src1 = h->mc.get_ref(pix1, &stride1, h->mb.pic.p_fref[1][a->l1.bi16x16.i_ref], h->mb.pic.i_stride[0], a->l1.bi16x16.mv[0], a->l1.bi16x16.mv[1],
                         16, 16, x264_weight_none);

    h->mc.avg[PIXEL_16x16](pix0, 16, src0, stride0, src1, stride1, h->mb.bipred_weight[a->l0.bi16x16.i_ref][a->l1.bi16x16.i_ref]);

    a->i_cost16x16bi =
        h->pixf.mbcmp[PIXEL_16x16](h->mb.pic.p_fenc[0], FENC_STRIDE, pix0, 16) + ref_costs + a->l0.bi16x16.cost_mv + a->l1.bi16x16.cost_mv;
    ...
    if (M32(a->l0.bi16x16.mv) | M32(a->l1.bi16x16.mv)) {
        int l0_mv_cost = a->l0.bi16x16.p_cost_mv[-a->l0.bi16x16.mvp[0]] + a->l0.bi16x16.p_cost_mv[-a->l0.bi16x16.mvp[1]];
        int l1_mv_cost = a->l1.bi16x16.p_cost_mv[-a->l1.bi16x16.mvp[0]] + a->l1.bi16x16.p_cost_mv[-a->l1.bi16x16.mvp[1]];
        ...
        int cost00 = h->pixf.mbcmp[PIXEL_16x16](h->mb.pic.p_fenc[0], FENC_STRIDE, pix0, 16) + ref_costs + l0_mv_cost + l1_mv_cost;
        ...
        if (h->mb.b_chroma_me && cost00 < a->i_cost16x16bi) {
            ...
            if (CHROMA444) {
                ...
                if (v_shift & MB_INTERLACED & a->l0.bi16x16.i_ref) {
                    ...
                    h->mc.mc_chroma(pixuv[0], pixuv[0] + 8, FENC_STRIDE, h->mb.pic.p_fref[0][a->l0.bi16x16.i_ref][4], h->mb.pic.i_stride[1], 0,
                                    0 + l0_mvy_offset, 8, 8);
                } else {
                    h->mc.load_deinterleave_chroma_fenc(pixuv[0], h->mb.pic.p_fref[0][a->l0.bi16x16.i_ref][4], h->mb.pic.i_stride[1], 16 >> v_shift);
                }

                if (v_shift & MB_INTERLACED & a->l1.bi16x16.i_ref) {
                    int l1_mvy_offset = (h->mb.i_mb_y & 1) * 4 - 2;
                    h->mc.mc_chroma(pixuv[1], pixuv[1] + 8, FENC_STRIDE, h->mb.pic.p_fref[1][a->l1.bi16x16.i_ref][4], h->mb.pic.i_stride[1], 0,
                                    0 + l1_mvy_offset, 8, 8);
                } else {
                    h->mc.load_deinterleave_chroma_fenc(pixuv[1], h->mb.pic.p_fref[1][a->l1.bi16x16.i_ref][4], h->mb.pic.i_stride[1], 16 >> v_shift);
                }
...
#define CHECK_NEIGHBOUR(i)                              \
    ...
    for (int l = 0; l < 2; l++) {
        x264_mb_analysis_list_t *lX = l ? &a->l1 : &a->l0;
        if (i_maxref[l] > 0 && lX->me16x16.i_ref == 0 && h->mb.i_mb_type_top > 0 && h->mb.i_mb_type_left[0] > 0) {
            i_maxref[l] = 0;
            CHECK_NEIGHBOUR(-8 - 1);
            CHECK_NEIGHBOUR(-8 + 0);
            CHECK_NEIGHBOUR(-8 + 2);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        /* BI mode */
        src[0] = h->mc.get_ref(pix[0], &stride[0], a->l0.me8x8[i].p_fref, a->l0.me8x8[i].i_stride[0], a->l0.me8x8[i].mv[0], a->l0.me8x8[i].mv[1], 8,
                               8, x264_weight_none);
        src[1] = h->mc.get_ref(pix[1], &stride[1], a->l1.me8x8[i].p_fref, a->l1.me8x8[i].i_stride[0], a->l1.me8x8[i].mv[0], a->l1.me8x8[i].mv[1], 8,
                               8, x264_weight_none);
        h->mc.avg[PIXEL_8x8](pix[0], 8, src[0], stride[0], src[1], stride[1], h->mb.bipred_weight[a->l0.me8x8[i].i_ref][a->l1.me8x8[i].i_ref]);

        a->i_satd8x8[2][i] = h->pixf.mbcmp[PIXEL_8x8](a->l0.me8x8[i].p_fenc[0], FENC_STRIDE, pix[0], 8);
        i_part_cost_bi = a->i_satd8x8[2][i] + a->l0.me8x8[i].cost_mv + a->l1.me8x8[i].cost_mv + a->l0.me8x8[i].i_ref_cost +
...
{
    pixel **p_fref[2] = {h->mb.pic.p_fref[0][a->l0.me16x16.i_ref], h->mb.pic.p_fref[1][a->l1.me16x16.i_ref]};
    ALIGNED_ARRAY_16(pixel, pix, [2], [8 * 8]);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int l = 0; l < 2; l++) {
            ...
            /* BI mode */
            src[l] = h->mc.get_ref(pix[l], &stride[l], m->p_fref, m->i_stride[0], m->mv[0], m->mv[1], 8, 8, x264_weight_none);
...
static void mb_analyse_inter_b16x8(x264_t *h, x264_mb_analysis_t *a, int i_best_satd)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        /* BI mode */
        src[0] = h->mc.get_ref(pix[0], &stride[0], a->l0.me16x8[i].p_fref, a->l0.me16x8[i].i_stride[0], a->l0.me16x8[i].mv[0], a->l0.me16x8[i].mv[1],
                               16, 8, x264_weight_none);
        src[1] = h->mc.get_ref(pix[1], &stride[1], a->l1.me16x8[i].p_fref, a->l1.me16x8[i].i_stride[0], a->l1.me16x8[i].mv[0], a->l1.me16x8[i].mv[1],
                               16, 8, x264_weight_none);
        h->mc.avg[PIXEL_16x8](pix[0], 16, src[0], stride[0], src[1], stride[1], h->mb.bipred_weight[a->l0.me16x8[i].i_ref][a->l1.me16x8[i].i_ref]);

        i_part_cost_bi = h->pixf.mbcmp[PIXEL_16x8](a->l0.me16x8[i].p_fenc[0], FENC_STRIDE, pix[0], 16) + a->l0.me16x8[i].cost_mv +
                         a->l1.me16x8[i].cost_mv + a->l0.me16x8[i].i_ref_cost + a->l1.me16x8[i].i_ref_cost;
        ...
        /* Early termination based on the current SATD score of partition[0]
           plus the estimated SATD score of partition[1] */
        if (a->b_early_terminate && (!i && i_part_cost + a->i_cost_est16x8[1] > i_best_satd * (16 + (!!a->i_mbrd + !!h->mb.i_psy_rd)) / 16)) {
...
static void mb_analyse_inter_b8x16(x264_t *h, x264_mb_analysis_t *a, int i_best_satd)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        /* BI mode */
        src[0] = h->mc.get_ref(pix[0], &stride[0], a->l0.me8x16[i].p_fref, a->l0.me8x16[i].i_stride[0], a->l0.me8x16[i].mv[0], a->l0.me8x16[i].mv[1],
                               8, 16, x264_weight_none);
        src[1] = h->mc.get_ref(pix[1], &stride[1], a->l1.me8x16[i].p_fref, a->l1.me8x16[i].i_stride[0], a->l1.me8x16[i].mv[0], a->l1.me8x16[i].mv[1],
                               8, 16, x264_weight_none);
        h->mc.avg[PIXEL_8x16](pix[0], 8, src[0], stride[0], src[1], stride[1], h->mb.bipred_weight[a->l0.me8x16[i].i_ref][a->l1.me8x16[i].i_ref]);

        i_part_cost_bi = h->pixf.mbcmp[PIXEL_8x16](a->l0.me8x16[i].p_fenc[0], FENC_STRIDE, pix[0], 8) + a->l0.me8x16[i].cost_mv +
                         a->l1.me8x16[i].cost_mv + a->l0.me8x16[i].i_ref_cost + a->l1.me8x16[i].i_ref_cost;
        ...
        /* Early termination based on the current SATD score of partition[0]
           plus the estimated SATD score of partition[1] */
        if (a->b_early_terminate && (!i && i_part_cost + a->i_cost_est8x16[1] > i_best_satd * (16 + (!!a->i_mbrd + !!h->mb.i_psy_rd)) / 16)) {
...
static inline void mb_analyse_transform(x264_t *h)
{
    if (x264_mb_transform_8x8_allowed(h) && h->param.analyse.b_transform_8x8 && !h->mb.b_lossless) {
        ...
        if (h->pixf.sa8d_satd[PIXEL_16x16]) {
            uint64_t cost = 0;
            for (int p = 0; p < plane_count; p++) {
                cost += h->pixf.sa8d_satd[PIXEL_16x16](h->mb.pic.p_fenc[p], FENC_STRIDE, h->mb.pic.p_fdec[p], FDEC_STRIDE);
            }
            i_cost8 = (uint32_t)cost;
            i_cost4 = (uint32_t)(cost >> 32);
        } else {
            for (int p = 0; p < plane_count; p++) {
                i_cost8 += h->pixf.sa8d[PIXEL_16x16](h->mb.pic.p_fenc[p], FENC_STRIDE, h->mb.pic.p_fdec[p], FDEC_STRIDE);
...
static inline void mb_analyse_qp_rd(x264_t *h, x264_mb_analysis_t *a)
{
    ...
    for (int direction = origcbp ? 1 : -1; direction >= -1; direction -= 2) {
        ...
        if ((h->mb.i_last_qp < orig_qp && direction == -1) || (h->mb.i_last_qp > orig_qp && direction == 1)) {
            threshold++;
        }
        h->mb.i_qp = orig_qp;
        failures = 0;
    ...
    if (h->mb.i_qp != orig_qp && h->param.analyse.b_transform_8x8 && x264_mb_transform_8x8_allowed(h)) {
        h->mb.b_transform_8x8 ^= 1;
        cost = rd_cost_mb(h, a->i_lambda2);
        if (cost > bcost) {
            h->mb.b_transform_8x8 ^= 1;
...
void x264_macroblock_analyse(x264_t *h)
{
    ...
    intra_analysis:
        ...
        if (analysis.b_force_intra) {
            ...
            /* Fast P_SKIP detection */
                ...
                else if (h->mb.i_mb_type_left[0] == P_SKIP || h->mb.i_mb_type_top == P_SKIP || h->mb.i_mb_type_topleft == P_SKIP ||
        ...
        skip_analysis:
            ...
            if ((flags & X264_ANALYSE_PSUB16x16) && (!analysis.b_early_terminate || analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost)) {
                i_type = P_8x8;
                i_partition = D_8x8;
            ...
            if ((flags & X264_ANALYSE_PSUB16x16) &&
                (!analysis.b_early_terminate || analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost + i_thresh16x8)) {
                int i_avg_mv_ref_cost = (analysis.l0.me8x8[2].cost_mv + analysis.l0.me8x8[2].i_ref_cost + analysis.l0.me8x8[3].cost_mv +
                                         analysis.l0.me8x8[3].i_ref_cost + 1) >>
                                        1;
                ...
                i_avg_mv_ref_cost = (analysis.l0.me8x8[1].cost_mv + analysis.l0.me8x8[1].i_ref_cost + analysis.l0.me8x8[3].cost_mv +
                                     analysis.l0.me8x8[3].i_ref_cost + 1) >>
                                    1;
            ...
            if (analysis.i_mbrd || !h->mb.i_subpel_refine) {
                ...
                for (int i8x8 = 0; i8x8 < 4; i8x8++) {
                    ...
                    case D_L0_4x4:
                        ...
                        i_cost += analysis.l0.me4x4[i8x8][0].cost + analysis.l0.me4x4[i8x8][1].cost + analysis.l0.me4x4[i8x8][2].cost +
        ...
        if (!b_skip) {
            ...
            if (analysis.i_mbrd && analysis.b_early_terminate && analysis.i_cost16x16direct <= i_cost * 33 / 32) {
                ...
                if (i_bskip_cost < analysis.i_rd16x16direct && i_bskip_cost < analysis.i_rd16x16bi && i_bskip_cost < analysis.l0.i_rd16x16 &&
            ...
            if (flags & X264_ANALYSE_BSUB16x16) {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    avg_l0_mv_ref_cost = (analysis.l0.me8x8[i * 2].cost_mv + analysis.l0.me8x8[i * 2].i_ref_cost +
                                          analysis.l0.me8x8[i * 2 + 1].cost_mv + analysis.l0.me8x8[i * 2 + 1].i_ref_cost + 1) >>
                                         1;
                    avg_l1_mv_ref_cost = (analysis.l1.me8x8[i * 2].cost_mv + analysis.l1.me8x8[i * 2].i_ref_cost +
                                          analysis.l1.me8x8[i * 2 + 1].cost_mv + analysis.l1.me8x8[i * 2 + 1].i_ref_cost + 1) >>
                                         1;
                    ...
                    avg_l0_mv_ref_cost = (analysis.l0.me8x8[i].cost_mv + analysis.l0.me8x8[i].i_ref_cost + analysis.l0.me8x8[i + 2].cost_mv +
                                          analysis.l0.me8x8[i + 2].i_ref_cost + 1) >>
                                         1;
                    avg_l1_mv_ref_cost = (analysis.l1.me8x8[i].cost_mv + analysis.l1.me8x8[i].i_ref_cost + analysis.l1.me8x8[i + 2].cost_mv +
                                          analysis.l1.me8x8[i + 2].i_ref_cost + 1) >>
                                         1;
    ...
    if (analysis.i_mbrd >= 2) {
        ...
        if (list >= 0 && h->mb.i_partition != D_16x16 && M32(&h->mb.cache.mv[list][x264_scan8[0]]) == M32(&h->mb.cache.mv[list][x264_scan8[12]]) &&
