...
#define COPY_CABAC h->mc.memcpy_aligned(&cabac_tmp.f8_bits_encoded, &h->cabac.f8_bits_encoded, sizeof(int) + (CHROMA444 ? 1024 + 12 : 460))
...
static ALWAYS_INLINE uint64_t cached_hadamard(x264_t *h, int size, int x, int y)
{
    ...
    int cache_index = (x >> hadamard_shift_x[size]) + (y >> hadamard_shift_y[size]) + hadamard_offset[size];
...
static ALWAYS_INLINE int cached_satd(x264_t *h, int size, int x, int y)
{
    ...
    int cache_index = (x >> satd_shift_x[size - PIXEL_8x4]) + (y >> satd_shift_y[size - PIXEL_8x4]) + satd_offset[size - PIXEL_8x4];
...
static inline int ssd_plane(x264_t *h, int size, int p, int x, int y)
{
    ...
    if (p == 0 && h->mb.i_psy_rd) {
        ...
        if (size <= PIXEL_8x8) {
            ...
            satd = abs((int32_t)fdec_acs - (int32_t)fenc_acs) + abs((int32_t)(fdec_acs >> 32) - (int32_t)(fenc_acs >> 32));
            satd >>= 1;
        } else {
...
static uint64_t rd_cost_subpart(x264_t *h, int i_lambda2, int i4, int i_pixel)
{
    ...
    if (CHROMA444) {
        int chromassd =
            ssd_plane(h, i_pixel, 1, block_idx_x[i4] * 4, block_idx_y[i4] * 4) + ssd_plane(h, i_pixel, 2, block_idx_x[i4] * 4, block_idx_y[i4] * 4);
...
static uint64_t rd_cost_i8x8(x264_t *h, int i_lambda2, int i8, int i_mode, pixel edge[4][32])
{
    ...
    if (CHROMA444) {
        int chromassd = ssd_plane(h, PIXEL_8x8, 1, (i8 & 1) * 8, (i8 >> 1) * 8) + ssd_plane(h, PIXEL_8x8, 2, (i8 & 1) * 8, (i8 >> 1) * 8);
        chromassd = ((uint64_t)chromassd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
        i_ssd += chromassd;
    }
...
static uint64_t rd_cost_chroma(x264_t *h, int i_lambda2, int i_mode, int b_dct)
{
    ...
    i_ssd = ssd_plane(h, chromapix, 1, 0, 0) + ssd_plane(h, chromapix, 2, 0, 0);
...
static NOINLINE int trellis_dc_shortcut(int sign_coef, int quant_coef, int unquant_mf, int coef_weight, int lambda2, uint8_t *cabac_state,
                                        int cost_sig)
{
...
static ALWAYS_INLINE int trellis_coef(int j, int const_level, int abs_level, int prefix, int suffix_cost, int node_ctx, int level1_ctx,
                                      int levelgt1_ctx, uint64_t ssd, int cost_siglast[3], trellis_node_t *nodes_cur, trellis_node_t *nodes_prev,
...
static NOINLINE int trellis_coef0_0(uint64_t ssd0, trellis_node_t *nodes_cur, trellis_node_t *nodes_prev, trellis_level_t *level_tree,
                                    int levels_used)
{
...
static NOINLINE int trellis_coef0_1(uint64_t ssd0, trellis_node_t *nodes_cur, trellis_node_t *nodes_prev, trellis_level_t *level_tree,
                                    int levels_used)
{
    for (int j = 1; j < 8; j++) {
...
#define COEF(const_level, ctx_hi, j, ...)                                                                                                 \
    if (!j || (int64_t)nodes_prev[j].score >= 0)                                                                                          \
        levels_used = trellis_coef(j, const_level, abs_level, prefix, suffix_cost, __VA_ARGS__, j ? ssd1 : ssd0, cost_siglast, nodes_cur, \
                                   nodes_prev, level_tree, levels_used, lambda2, level_state);                                            \
    else if (!ctx_hi)                                                                                                                     \
        return levels_used;

static NOINLINE int trellis_coef1_0(uint64_t ssd0, uint64_t ssd1, int cost_siglast[3], trellis_node_t *nodes_cur, trellis_node_t *nodes_prev,
                                    trellis_level_t *level_tree, int levels_used, int lambda2, uint8_t *level_state)
{
...
static NOINLINE int trellis_coef1_1(uint64_t ssd0, uint64_t ssd1, int cost_siglast[3], trellis_node_t *nodes_cur, trellis_node_t *nodes_prev,
                                    trellis_level_t *level_tree, int levels_used, int lambda2, uint8_t *level_state)
{
    int abs_level = 1, prefix = 1, suffix_cost = 0;
    COEF(1, 1, 1, 2, 2, 0);
...
static NOINLINE int trellis_coefn_0(int abs_level, uint64_t ssd0, uint64_t ssd1, int cost_siglast[3], trellis_node_t *nodes_cur,
                                    trellis_node_t *nodes_prev, trellis_level_t *level_tree, int levels_used, int lambda2, uint8_t *level_state,
                                    int levelgt1_ctx)
{
    int prefix = X264_MIN(abs_level - 1, 14);
...
static NOINLINE int trellis_coefn_1(int abs_level, uint64_t ssd0, uint64_t ssd1, int cost_siglast[3], trellis_node_t *nodes_cur,
                                    trellis_node_t *nodes_prev, trellis_level_t *level_tree, int levels_used, int lambda2, uint8_t *level_state,
                                    int levelgt1_ctx)
{
    int prefix = X264_MIN(abs_level - 1, 14);
...
static ALWAYS_INLINE int quant_trellis_cabac(x264_t *h, dctcoef *dct, udctcoef *quant_mf, udctcoef *quant_bias, const int *unquant_mf,
                                             const uint8_t *zigzag, int ctx_block_cat, int lambda2, int b_ac, int b_chroma, int dc, int num_coefs,
                                             int idx)
{
    ...
    if (last_nnz == 0 && !dc) {
        int cost_sig = x264_cabac_size_decision_noup2(&cabac_state_sig[0], 1) + x264_cabac_size_decision_noup2(&cabac_state_last[0], 1);
...
#define TRELLIS_ARGS \
    unquant_mf, zigzag, lambda2, last_nnz, orig_coefs, quant_coefs, dct, cabac_state_sig, cabac_state_last, level_state0, level_state1
    if (num_coefs == 16 && !dc) {
...
#define TRELLIS_LOOP(ctx_hi)                                                                                                                         \
    for (; i >= b_ac; i--) {                                                                                                                         \
        ...
        if (!quant_coefs[i]) {                                                                                                                       \
            ...
            if (!ctx_hi) {                                                                                                                           \
                int sigindex = !dc && num_coefs == 64             ? x264_significant_coeff_flag_offset_8x8[b_interlaced][i]                          \
                               : b_chroma && dc && num_coefs == 8 ? x264_coeff_flag_offset_chroma_422_dc[i]                                          \
                                                                  : i;                                                                               \
                uint64_t cost_sig0 =                                                                                                                 \
                    x264_cabac_size_decision_noup2(&cabac_state_sig[sigindex], 0) * (uint64_t)lambda2 >> (CABAC_SIZE_BITS - LAMBDA_BITS);            \
        ...
        case 1:                                                                                                                                      \
            ...
            levels_used = trellis_coef1_##ctx_hi(ssd0[1] - ssd1[0], ssd1[1] - ssd1[0], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, \
                                                 lambda2, level_state);                                                                              \
            goto next##ctx_hi;                                                                                                                       \
        case 2:                                                                                                                                      \
            levels_used =                                                                                                                            \
                trellis_coef1_##ctx_hi(ssd0[0], ssd1[0], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2, level_state);        \
            levels_used = trellis_coefn_##ctx_hi(q, ssd0[1], ssd1[1], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2,         \
                                                 level_state, levelgt1_ctx);                                                                         \
            goto next1;                                                                                                                              \
        default:                                                                                                                                     \
            levels_used = trellis_coefn_##ctx_hi(q - 1, ssd0[0], ssd1[0], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2,     \
                                                 level_state, levelgt1_ctx);                                                                         \
            levels_used = trellis_coefn_##ctx_hi(q, ssd0[1], ssd1[1], cost_siglast, nodes_cur, nodes_prev, level_tree, levels_used, lambda2,         \
                                                 level_state, levelgt1_ctx);                                                                         \
            goto next1;                                                                                                                              \
...
static ALWAYS_INLINE int quant_trellis_cavlc(x264_t *h, dctcoef *dct, const udctcoef *quant_mf, const int *unquant_mf, const uint8_t *zigzag,
                                             int ctx_block_cat, int lambda2, int b_ac, int b_chroma, int dc, int num_coefs, int idx, int b_8x8)
{
    ...
    int i, j;
    const int f = 1 << 15;
    int nC = b_chroma && dc ? 3 + (num_coefs >> 2) : ct_index[x264_mb_predict_non_zero_code(h, !b_chroma && dc ? (idx - LUMA_DC) * 16 : idx)];
...
{
    if (h->param.b_cabac) {
        return quant_trellis_cabac(h, dct, h->quant4_mf[i_quant_cat][i_qp], h->quant4_bias0[i_quant_cat][i_qp], h->unquant4_mf[i_quant_cat][i_qp],
                                   x264_zigzag_scan4[MB_INTERLACED], ctx_block_cat, h->mb.i_trellis_lambda2[0][b_intra], 0, 0, 1, 16, idx);
    }

    return quant_trellis_cavlc(h, dct, h->quant4_mf[i_quant_cat][i_qp], h->unquant4_mf[i_quant_cat][i_qp], x264_zigzag_scan4[MB_INTERLACED],
...
int x264_quant_chroma_dc_trellis(x264_t *h, dctcoef *dct, int i_qp, int b_intra, int idx)
{
    ...
    if (h->param.b_cabac) {
        return quant_trellis_cabac(h, dct, h->quant4_mf[quant_cat][i_qp], h->quant4_bias0[quant_cat][i_qp], h->unquant4_mf[quant_cat][i_qp], zigzag,
                                   DCT_CHROMA_DC, h->mb.i_trellis_lambda2[1][b_intra], 0, 1, 1, num_coefs, idx);
    }

    return quant_trellis_cavlc(h, dct, h->quant4_mf[quant_cat][i_qp], h->unquant4_mf[quant_cat][i_qp], zigzag, DCT_CHROMA_DC,
                               h->mb.i_trellis_lambda2[1][b_intra], 0, 1, 1, num_coefs, idx, 0);
}

int x264_quant_4x4_trellis(x264_t *h, dctcoef *dct, int i_quant_cat, int i_qp, int ctx_block_cat, int b_intra, int b_chroma, int idx)
{
    static const uint8_t ctx_ac[14] = {0, 1, 0, 0, 1, 0, 0, 1, 0, 0, 0, 1, 0, 0};
    int b_ac = ctx_ac[ctx_block_cat];
    if (h->param.b_cabac) {
        return quant_trellis_cabac(h, dct, h->quant4_mf[i_quant_cat][i_qp], h->quant4_bias0[i_quant_cat][i_qp], h->unquant4_mf[i_quant_cat][i_qp],
                                   x264_zigzag_scan4[MB_INTERLACED], ctx_block_cat, h->mb.i_trellis_lambda2[b_chroma][b_intra], b_ac, b_chroma, 0, 16,
                                   idx);
    }
...
int x264_quant_8x8_trellis(x264_t *h, dctcoef *dct, int i_quant_cat, int i_qp, int ctx_block_cat, int b_intra, int b_chroma, int idx)
{
    if (h->param.b_cabac) {
        return quant_trellis_cabac(h, dct, h->quant8_mf[i_quant_cat][i_qp], h->quant8_bias0[i_quant_cat][i_qp], h->unquant8_mf[i_quant_cat][i_qp],
                                   x264_zigzag_scan8[MB_INTERLACED], ctx_block_cat, h->mb.i_trellis_lambda2[b_chroma][b_intra], 0, b_chroma, 0, 64,
                                   idx);
    }
    ...
    int nzaccum = 0;
    for (int i = 0; i < 4; i++) {
        int nz = quant_trellis_cavlc(h, dct, h->quant8_mf[i_quant_cat][i_qp], h->unquant8_mf[i_quant_cat][i_qp], x264_zigzag_scan8[MB_INTERLACED],
