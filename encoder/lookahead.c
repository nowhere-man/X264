...
static void lookahead_update_last_nonb(x264_t *h, x264_frame_t *new_nonb)
{
    ...
    log_trace("[lookahead][update_last_nonb]new_nonb->i_frame:%d,new_nonb->i_type:%d,new_nonb->i_reference_count:%d", new_nonb->i_frame,
              new_nonb->i_type, new_nonb->i_reference_count);
}
...
int x264_lookahead_init(x264_t *h, int i_slicetype_length)
{
    ...
    look->b_analyse_keyframe = (h->param.rc.b_mb_tree || (h->param.rc.i_vbv_buffer_size && h->param.rc.i_lookahead)) && !h->param.rc.b_stat_read;
    look->i_slicetype_length = i_slicetype_length;

    /* init frame lists */
    if (x264_sync_frame_list_init(&look->ifbuf, h->param.i_sync_lookahead + 3) || x264_sync_frame_list_init(&look->next, h->frames.i_delay + 3) ||
...
void x264_lookahead_get_frames(x264_t *h)
{
    ...
    if (h->param.i_sync_lookahead) { /* We have a lookahead thread, so get frames from there */
        ...
        for (size_t i = 0; i < h->lookahead->ofbuf.i_size; i++) {
            log_trace("[lookahead]finished,h->lookahead->ofbuf.list[%d]->i_pts=%d,h->lookahead->ofbuf.list[%d]->i_type=%d", i,
                      h->lookahead->ofbuf.list[i]->i_pts, i, h->lookahead->ofbuf.list[i]->i_type);
        }
        /* For MB-tree and VBV lookahead, we have to perform propagation analysis on I-frames too. */
        if (h->lookahead->b_analyse_keyframe && IS_X264_TYPE_I(h->lookahead->last_nonb->i_type)) {
            x264_slicetype_analyse(h, shift_frames);
