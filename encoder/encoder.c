...
static int encoder_frame_end(x264_t *h, x264_t *thread_current, x264_nal_t **pp_nal, int *pi_nal, x264_picture_t *pic_out);
...
/* Fill "default" values */
static void slice_header_init(x264_t *h, x264_slice_header_t *sh, x264_sps_t *sps, x264_pps_t *pps, int i_idr_pic_id, int i_frame, int i_qp)
{
    ...
    h->mb.b_direct_auto_write =
        h->param.analyse.i_direct_mv_pred == X264_DIRECT_PRED_AUTO && h->param.i_bframe && (h->param.rc.b_stat_write || !h->param.rc.b_stat_read);
...
{
    if ((b_cabac && (h->cabac.p_end - h->cabac.p < size)) || (h->out.bs.p_end - h->out.bs.p < size)) {
...
#define MAX_RESOLUTION 16384
    if (h->param.i_width <= 0 || h->param.i_height <= 0 || h->param.i_width > MAX_RESOLUTION || h->param.i_height > MAX_RESOLUTION) {
    ...
    if (i_csp <= X264_CSP_NONE || i_csp >= X264_CSP_MAX) {
        x264_log(h, X264_LOG_ERROR,
                 "invalid CSP (only I400/I420/YV12/NV12/NV21/I422/YV16/NV16/YUYV/UYVY/"
    ...
    if (h->param.crop_rect.i_left < 0 || h->param.crop_rect.i_left >= h->param.i_width || h->param.crop_rect.i_right < 0 ||
        h->param.crop_rect.i_right >= h->param.i_width || h->param.crop_rect.i_top < 0 || h->param.crop_rect.i_top >= h->param.i_height ||
        ...
        x264_log(h, X264_LOG_ERROR, "invalid crop-rect %d,%d,%d,%d\n", h->param.crop_rect.i_left, h->param.crop_rect.i_top,
                 h->param.crop_rect.i_right, h->param.crop_rect.i_bottom);
        return -1;
    }
    if (h->param.crop_rect.i_left % w_mod || h->param.crop_rect.i_right % w_mod || h->param.crop_rect.i_top % h_mod ||
        h->param.crop_rect.i_bottom % h_mod) {
        x264_log(h, X264_LOG_ERROR, "crop-rect %d,%d,%d,%d not divisible by %dx%d\n", h->param.crop_rect.i_left, h->param.crop_rect.i_top,
                 h->param.crop_rect.i_right, h->param.crop_rect.i_bottom, w_mod, h_mod);
        return -1;
    }
    ...
    if (h->param.i_frame_packing == 7 && ((h->param.i_width - h->param.crop_rect.i_left - h->param.crop_rect.i_right) % 3 ||
    ...
    if (h->param.content_light_level.b_cll && (h->param.content_light_level.i_max_cll > UINT16_MAX || h->param.content_light_level.i_max_cll < 0 ||
    ...
    if (h->param.i_avcintra_class) {
        ...
        for (i = 0; i < 7; i++) {
            if (avcintra_lut[type][res][i].fps_num == fps_num && avcintra_lut[type][res][i].fps_den == fps_den &&
        ...
        if (i == 7) {
            x264_log(h, X264_LOG_ERROR, "FPS %d/%d%c not compatible with AVC-Intra %d\n", h->param.i_fps_num, h->param.i_fps_den,
                     PARAM_INTERLACED ? 'i' : 'p', h->param.i_avcintra_class);
            return -1;
        }
        ...
        h->param.rc.i_vbv_max_bitrate = h->param.rc.i_bitrate = h->param.rc.i_vbv_buffer_size * fps_num / fps_den;
    ...
    if (h->param.rc.i_vbv_buffer_size) {
        ...
        } else if (h->param.rc.i_vbv_max_bitrate < h->param.rc.i_bitrate && h->param.rc.i_rc_method == X264_RC_ABR) {
    ...
    if ((!h->param.b_intra_refresh && h->param.i_keyint_max != X264_KEYINT_MAX_INFINITE) && !h->param.rc.i_lookahead && h->param.rc.b_mb_tree) {
    ...
    if (h->param.i_nal_hrd == X264_NAL_HRD_CBR && (h->param.rc.i_bitrate != h->param.rc.i_vbv_max_bitrate || !h->param.rc.i_vbv_max_bitrate)) {
...
static void mbcmp_init(x264_t *h)
{
    ...
    h->pixf.intra_mbcmp_x9_4x4 = h->param.b_cpu_independent || h->mb.b_lossless ? NULL : satd ? h->pixf.intra_satd_x9_4x4 : h->pixf.intra_sad_x9_4x4;
    h->pixf.intra_mbcmp_x9_8x8 = h->param.b_cpu_independent || h->mb.b_lossless ? NULL : satd ? h->pixf.intra_sa8d_x9_8x8 : h->pixf.intra_sad_x9_8x8;
...
x264_t *x264_encoder_open(x264_param_t *param, void *api)
{
    ...
    h->frames.b_have_lowres =
        !h->param.rc.b_stat_read && (h->param.rc.i_rc_method == X264_RC_ABR || h->param.rc.i_rc_method == X264_RC_CRF || h->param.i_bframe_adaptive ||
                                     h->param.i_scenecut_threshold || h->param.rc.b_mb_tree || h->param.analyse.i_weighted_pred);
    ...
    for (int i = 0; x264_cpu_names[i].flags; i++) {
        ...
        if (!strcmp(x264_cpu_names[i].name, "LZCNT") && (h->param.cpu & X264_CPU_BMI1)) {
            continue;
        }
        if (!strcmp(x264_cpu_names[i].name, "BMI1") && (h->param.cpu & X264_CPU_BMI2)) {
            continue;
        }
        if (!strcmp(x264_cpu_names[i].name, "FMA4") && (h->param.cpu & X264_CPU_FMA3)) {
            continue;
        }
    ...
    h->out.i_nal = 0;
    h->out.i_bitstream =
        x264_clip3f(h->param.i_width * h->param.i_height * 4 *
    ...
    if (h->param.i_threads > 1 && x264_threadpool_init(&h->threadpool, h->param.i_threads)) {
        goto fail;
    }
    if (h->param.i_lookahead_threads > 1 && x264_threadpool_init(&h->lookaheadpool, h->param.i_lookahead_threads)) {
        goto fail;
    }
    ...
    *h->reconfig_h = *h;
    ...
    const char *profile = h->sps->i_profile_idc == PROFILE_BASELINE  ? "Constrained Baseline"
                          ...
                          : h->sps->i_profile_idc == PROFILE_HIGH10  ? (h->sps->b_constraint_set3 ? "High 10 Intra" : "High 10")
                          : h->sps->i_profile_idc == PROFILE_HIGH422 ? (h->sps->b_constraint_set3 ? "High 4:2:2 Intra" : "High 4:2:2")
                          : h->sps->b_constraint_set3                ? "High 4:4:4 Intra"
...
#define COPY(var) h->param.var = param->var
    ...
    if (h->param.rc.i_vbv_max_bitrate > 0 && h->param.rc.i_vbv_buffer_size > 0 && param->rc.i_vbv_max_bitrate > 0 &&
...
static int check_encapsulated_buffer(x264_t *h, x264_t *h0, int start, int64_t previous_nal_size, int64_t necessary_size)
{
    ...
    for (int list = 0; list <= (h->sh.i_type == SLICE_TYPE_B); list++) {
        for (int i = 0; i < h->i_ref[list] - 1; i++) {
            ...
            if (h->sh.i_type == SLICE_TYPE_P ? framenum_diff > 0 : list == 1 ? poc_diff < 0 : poc_diff > 0) {
                log_trace("[ref_check_order][reorder]h->sh.i_type:%d,list:%d,i:%d,framenum_diff:%d,poc_diff:%d", h->sh.i_type, list, i, framenum_diff,
                          poc_diff);
...
static inline int reference_distance(x264_t *h, x264_frame_t *frame)
{
    if (h->param.i_frame_packing == 5) {
        return abs((h->fenc->i_frame & ~1) - (frame->i_frame & ~1)) + ((h->fenc->i_frame & 1) != (frame->i_frame & 1));
    } else {
...
static inline void reference_build_list(x264_t *h, int i_poc)
{
    ...
    for (int list = 0; list < 2; list++) {
        ...
        do {
            ...
            for (int i = 0; i < h->i_ref[list] - 1; i++) {
                ...
                if (list ? h->fref[list][i + 1]->i_poc < h->fref_nearest[list]->i_poc : h->fref[list][i + 1]->i_poc > h->fref_nearest[list]->i_poc) {
    ...
    for (int i = 0; i < h->i_ref[0]; i++) {
        log_trace("[ref_build]h->fref[0][%d]->i_frame=%d, h->fref[0][%d]->i_poc=%d", i, h->fref[0][i]->i_frame, i, h->fref[0][i]->i_poc);
    }
    for (int i = 0; i < h->i_ref[1]; i++) {
        log_trace("[ref_build]h->fref[1][%d]->i_frame=%d, h->fref[1][%d]->i_poc=%d", i, h->fref[1][i]->i_frame, i, h->fref[1][i]->i_poc);
    }
}
...
static void fdec_filter_row(x264_t *h, int mb_y, int pass)
{
    ...
    if (PARAM_INTERLACED && (!h->param.b_sliced_threads || pass == 1)) {
        for (int p = 0; p < h->fdec->i_plane; p++) {
            for (int i = minpix_y >> (CHROMA_V_SHIFT && p); i < maxpix_y >> (CHROMA_V_SHIFT && p); i++) {
                memcpy(h->fdec->plane_fld[p] + i * h->fdec->i_stride[p], h->fdec->plane[p] + i * h->fdec->i_stride[p],
    ...
    if (b_measure_quality) {
        ...
        if (h->param.analyse.b_psnr) {
            for (int p = 0; p < (CHROMA444 ? 3 : 1); p++) {
                h->stat.frame.i_ssd[p] += x264_pixel_ssd_wxh(&h->pixf, h->fdec->plane[p] + minpix_y * h->fdec->i_stride[p], h->fdec->i_stride[p],
            ...
            if (!CHROMA444) {
                ...
                x264_pixel_ssd_nv12(&h->pixf, h->fdec->plane[1] + (minpix_y >> v_shift) * h->fdec->i_stride[1], h->fdec->i_stride[1],
                                    h->fenc->plane[1] + (minpix_y >> v_shift) * h->fenc->i_stride[1], h->fenc->i_stride[1], h->param.i_width >> 1,
                                    (maxpix_y - minpix_y) >> v_shift, &ssd_u, &ssd_v);
        ...
        if (h->param.analyse.b_ssim) {
            ...
            h->stat.frame.f_ssim += x264_pixel_ssim_wxh(&h->pixf, h->fdec->plane[0] + 2 + minpix_y * h->fdec->i_stride[0], h->fdec->i_stride[0],
...
static inline int reference_update(x264_t *h)
{
    ...
    if (!h->fdec->b_kept_as_ref) {
        log_trace("[ref_upd][!b_kept_as_ref]h->fdec->i_frame:%d,h->fdec->i_poc:%d,h->fdec->i_type:%d", h->fdec->i_frame, h->fdec->i_poc,
                  h->fdec->i_type);
        if (h->i_thread_frames > 1) {
    ...
    for (int i = 0; i < h->sh.i_mmco_command_count; i++) {
        for (int j = 0; h->frames.reference[j]; j++) {
            if (h->frames.reference[j]->i_poc == h->sh.mmco[i].i_poc) {
                log_trace("[ref_upd][mmco]h->frames.reference[%d]->i_frame:%d,h->frames.reference[%d]->i_poc:%d", j, h->frames.reference[j]->i_frame,
                          j, h->frames.reference[j]->i_poc);
                x264_frame_push_unused(h, x264_frame_shift(&h->frames.reference[j]));
            }
        }
    }

    log_trace("[ref_upd][move fdec to ref]h->fdec->i_frame:%d,h->fdec->i_poc:%d,h->fdec->i_type:%d", h->fdec->i_frame, h->fdec->i_poc,
              h->fdec->i_type);
    ...
    for (int i = 0; h->frames.reference[i]; i++) {
        log_trace("[ref_upd][reference]h->frames.reference[%d]->i_frame:%d,h->frames.reference[%d]->i_poc:%d,h->frames.reference[%d]->i_type:%d", i,
                  h->frames.reference[i]->i_frame, i, h->frames.reference[i]->i_poc, i, h->frames.reference[i]->i_type);
    }
...
static inline void reference_hierarchy_reset(x264_t *h)
{
    ...
    for (ref = 0; h->frames.reference[ref]; ref++) {
        if ((h->param.i_bframe_pyramid == X264_B_PYRAMID_STRICT && h->frames.reference[ref]->i_type == X264_TYPE_BREF) ||
            (h->frames.reference[ref]->i_poc < h->frames.i_poc_last_open_gop && h->sh.i_type != SLICE_TYPE_B)) {
    ...
    for (int i = 0; i < h->sh.i_mmco_command_count; i++) {
        log_trace("[hierarchy_reset]h->sh.mmco[%d].i_difference_of_pic_nums, h->sh.mmco[%d].i_poc", h->sh.mmco[i].i_difference_of_pic_nums,
                  h->sh.mmco[i].i_poc);
    }
                    ...
                    for (int i = 0; i < 4; i++) {
                        if (h->mb.i_cbp_luma & (1 << i)) {
                            for (int p = 0; p < 3; p++) {
                                int s8 = i * 4 + p * 16;
                                int nnz8x8 =
                                    M16(&h->mb.cache.non_zero_count[x264_scan8[s8] + 0]) | M16(&h->mb.cache.non_zero_count[x264_scan8[s8] + 8]);
        ...
        if (!i_slice_num || !x264_frame_new_slice(h, h->fdec)) {
            if (h->param.i_slice_max_mbs) {
                if (SLICE_MBAFF) {
                    ...
                    int last_mbaff = 2 * (h->sh.i_first_mb % h->mb.i_mb_width) + h->mb.i_mb_width * (h->sh.i_first_mb / h->mb.i_mb_width) +
...
int x264_encoder_encode(x264_t *h, x264_nal_t **pp_nal, int *pi_nal, x264_picture_t *pic_in, x264_picture_t *pic_out)
{
    ...
    *pp_nal = NULL;
    ...
    if (pic_in != NULL) {
        ...
        if (h->param.i_width != 16 * h->mb.i_mb_width || h->param.i_height != 16 * h->mb.i_mb_height) {
    ...
    log_trace("[encoder_encode]ENCODE BEGIN! h->i_frame:%d,fenc->i_type=%d,fenc->i_frame=%d", h->i_frame, h->fenc->i_type, h->fenc->i_frame);
    ...
    h->sh.i_mmco_command_count = h->sh.i_mmco_remove_from_end = 0;
...
static int encoder_frame_end(x264_t *h, x264_t *thread_current, x264_nal_t **pp_nal, int *pi_nal, x264_picture_t *pic_out)
{
    ...
    if (h->i_thread_frames > 1 && h->fenc->b_keyframe && h->sps->vui.b_nal_hrd_parameters_present) {
        ...
        int idx = 0;
        while (h->out.nal[idx].i_type == NAL_AUD || h->out.nal[idx].i_type == NAL_SPS || h->out.nal[idx].i_type == NAL_PPS) {
            idx++;
        }
    ...
    *pp_nal = h->out.nal;
    ...
    if (h->param.analyse.b_psnr) {
        ...
        snprintf(psz_message, 80, " PSNR Y:%5.2f U:%5.2f V:%5.2f", pic_out->prop.f_psnr[0], pic_out->prop.f_psnr[1], pic_out->prop.f_psnr[2]);
    }
    ...
    x264_log(h, X264_LOG_DEBUG, "frame=%4d QP=%.2f NAL=%d Slice:%c Poc:%-3d I:%-4d P:%-4d SKIP:%-4d size=%d bytes%s\n", h->i_frame,
             h->fdec->f_qp_avg_aq, h->i_nal_ref_idc, h->sh.i_type == SLICE_TYPE_I ? 'I' : (h->sh.i_type == SLICE_TYPE_P ? 'P' : 'B'), h->fdec->i_poc,
             h->stat.frame.i_mb_count_i, h->stat.frame.i_mb_count_p, h->stat.frame.i_mb_count_skip, frame_size, psz_message);
...
{
    intra += sprintf(intra, "I16..4%s: %4.1f%% %4.1f%% %4.1f%%", b_print_pcm ? "..PCM" : "", i_mb_count[I_16x16] / i_count,
                     i_mb_count[I_8x8] / i_count, i_mb_count[I_4x4] / i_count);
    if (b_print_pcm) {
...
void x264_encoder_close(x264_t *h)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (h->stat.i_frame_count[i_slice] > 0) {
            ...
            if (h->param.analyse.b_psnr) {
                x264_log(h, X264_LOG_INFO, "frame %c:%-5d Avg QP:%5.2f  size:%6.0f  PSNR Mean Y:%5.2f U:%5.2f V:%5.2f Avg:%5.2f Global:%5.2f\n",
                         slice_type_to_char[i_slice], i_count, h->stat.f_frame_qp[i_slice] / i_count, (double)h->stat.i_frame_size[i_slice] / i_count,
                         h->stat.f_psnr_mean_y[i_slice] / dur, h->stat.f_psnr_mean_u[i_slice] / dur, h->stat.f_psnr_mean_v[i_slice] / dur,
                         h->stat.f_psnr_average[i_slice] / dur, calc_psnr(h->stat.f_ssd_global[i_slice], dur * i_yuv_size));
            } else {
                x264_log(h, X264_LOG_INFO, "frame %c:%-5d Avg QP:%5.2f  size:%6.0f\n", slice_type_to_char[i_slice], i_count,
                         h->stat.f_frame_qp[i_slice] / i_count, (double)h->stat.i_frame_size[i_slice] / i_count);
            }
        }
    }
    ...
    if (h->stat.i_frame_count[SLICE_TYPE_P] > 0) {
        ...
        x264_log(h, X264_LOG_INFO, "mb P  %s  P16..4: %4.1f%% %4.1f%% %4.1f%% %4.1f%% %4.1f%%    skip:%4.1f%%\n", buf,
                 i_mb_size[PIXEL_16x16] / (i_count * 4), (i_mb_size[PIXEL_16x8] + i_mb_size[PIXEL_8x16]) / (i_count * 4),
                 i_mb_size[PIXEL_8x8] / (i_count * 4), (i_mb_size[PIXEL_8x4] + i_mb_size[PIXEL_4x8]) / (i_count * 4),
                 i_mb_size[PIXEL_4x4] / (i_count * 4), i_mb_count[P_SKIP] / i_count);
    }
    if (h->stat.i_frame_count[SLICE_TYPE_B] > 0) {
        ...
        sprintf(buf + strlen(buf), "  B16..8: %4.1f%% %4.1f%% %4.1f%%  direct:%4.1f%%  skip:%4.1f%%", i_mb_size[PIXEL_16x16] / (i_count * 4),
                (i_mb_size[PIXEL_16x8] + i_mb_size[PIXEL_8x16]) / (i_count * 4), i_mb_size[PIXEL_8x8] / (i_count * 4), i_mb_count[B_DIRECT] / i_count,
                i_mb_count[B_SKIP] / i_count);
        if (i_mb_list_count != 0) {
            sprintf(buf + strlen(buf), "  L0:%4.1f%% L1:%4.1f%% BI:%4.1f%%", list_count[0] / i_mb_list_count, list_count[1] / i_mb_list_count,
...
#define SUM3b(p, o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        const int i_count = h->stat.i_frame_count[SLICE_TYPE_I] + h->stat.i_frame_count[SLICE_TYPE_P] + h->stat.i_frame_count[SLICE_TYPE_B];
        int64_t i_mb_count = (int64_t)i_count * h->mb.i_mb_count;
        int64_t i_inter = i_mb_count - i_skip - i_all_intra;
        const double duration =
            h->stat.f_frame_duration[SLICE_TYPE_I] + h->stat.f_frame_duration[SLICE_TYPE_P] + h->stat.f_frame_duration[SLICE_TYPE_B];
        ...
        if ((h->param.analyse.i_direct_mv_pred == X264_DIRECT_PRED_AUTO || (h->stat.i_direct_frames[0] && h->stat.i_direct_frames[1])) &&
        ...
        if (CHROMA_FORMAT) {
            ...
            if (i_mb_count != i_all_intra) {
                sprintf(buf, " inter: %.1f%% %.1f%% %.1f%%", h->stat.i_mb_cbp[1] * 100.0 / ((i_mb_count - i_all_intra) * 4),
            ...
            x264_log(h, X264_LOG_INFO, "coded y,%s,%s intra: %.1f%% %.1f%% %.1f%%%s\n", CHROMA444 ? "u" : "uvDC", CHROMA444 ? "v" : "uvAC",
                     h->stat.i_mb_cbp[0] * 100.0 / (i_all_intra * 4), h->stat.i_mb_cbp[2] * 100.0 / (i_all_intra * csize),
        ...
        if (sum_pred_modes[0]) {
            x264_log(h, X264_LOG_INFO, "i16 v,h,dc,p: %2.0f%% %2.0f%% %2.0f%% %2.0f%%\n", fixed_pred_modes[0][0] * 100.0 / sum_pred_modes[0],
                     fixed_pred_modes[0][1] * 100.0 / sum_pred_modes[0], fixed_pred_modes[0][2] * 100.0 / sum_pred_modes[0],
        ...
        for (int i = 1; i <= 2; i++) {
            ...
            if (sum_pred_modes[i]) {
                x264_log(h, X264_LOG_INFO,
                         ...
                         fixed_pred_modes[i][0] * 100.0 / sum_pred_modes[i], fixed_pred_modes[i][1] * 100.0 / sum_pred_modes[i],
                         fixed_pred_modes[i][2] * 100.0 / sum_pred_modes[i], fixed_pred_modes[i][3] * 100.0 / sum_pred_modes[i],
                         fixed_pred_modes[i][4] * 100.0 / sum_pred_modes[i], fixed_pred_modes[i][5] * 100.0 / sum_pred_modes[i],
                         fixed_pred_modes[i][6] * 100.0 / sum_pred_modes[i], fixed_pred_modes[i][7] * 100.0 / sum_pred_modes[i],
        ...
        if (sum_pred_modes[3] && !CHROMA444) {
            x264_log(h, X264_LOG_INFO, "i8c dc,h,v,p: %2.0f%% %2.0f%% %2.0f%% %2.0f%%\n", fixed_pred_modes[3][0] * 100.0 / sum_pred_modes[3],
                     fixed_pred_modes[3][1] * 100.0 / sum_pred_modes[3], fixed_pred_modes[3][2] * 100.0 / sum_pred_modes[3],
        ...
        if (h->param.analyse.b_psnr) {
            x264_log(h, X264_LOG_INFO, "PSNR Mean Y:%6.3f U:%6.3f V:%6.3f Avg:%6.3f Global:%6.3f kb/s:%.2f\n", SUM3(h->stat.f_psnr_mean_y) / duration,
                     SUM3(h->stat.f_psnr_mean_u) / duration, SUM3(h->stat.f_psnr_mean_v) / duration, SUM3(h->stat.f_psnr_average) / duration,
                     calc_psnr(SUM3(h->stat.f_ssd_global), duration * i_yuv_size), f_bitrate);
        } else {
...
int x264_encoder_maximum_delayed_frames(x264_t *h) { return h->frames.i_delay; }
