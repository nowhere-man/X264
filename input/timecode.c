    ...
    } else /* tcfv == 2 */
    {
        ...
        if (fgets(buff, sizeof(buff), tcfile_in) != NULL) {
            ...
            for (num = 1; num < timecodes_num && fgets(buff, sizeof(buff), tcfile_in) != NULL;) {
                ...
                FAIL_IF_ERROR(ret != 1 || timecodes[num] <= timecodes[num - 1], "invalid input tcfile for frame %d\n", num);
                ++num;
            }
        }
...
static int open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt)
{
    ...
    if (opt->timebase) {
        ...
        FAIL_IF_ERROR(h->timebase_num > UINT32_MAX || h->timebase_den > UINT32_MAX, "timebase you specified exceeds H.264 maximum\n");
    }
