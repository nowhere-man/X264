...
static int open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt)
{
    ...
    if ((opt->output_csp == X264_CSP_I400 && !AVS_IS_Y(vi)) || (opt->output_csp == X264_CSP_I420 && !AVS_IS_420(vi)) ||
        (opt->output_csp == X264_CSP_I422 && !AVS_IS_422(vi)) || (opt->output_csp == X264_CSP_I444 && !AVS_IS_444(vi)) ||
        ...
        if (opt->output_csp != X264_CSP_I400) {
            FAIL_IF_ERROR(opt->output_csp < X264_CSP_I444 && (vi->width & 1), "input clip width not divisible by 2 (%dx%d)\n", vi->width, vi->height);
            FAIL_IF_ERROR(opt->output_csp == X264_CSP_I420 && info->interlaced && (vi->height & 3), "input clip height not divisible by 4 (%dx%d)\n",
                          vi->width, vi->height);
