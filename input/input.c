...
int x264_cli_csp_is_invalid(int csp)
{
    ...
    return csp_mask <= X264_CSP_NONE || csp_mask >= X264_CSP_CLI_MAX || csp_mask == X264_CSP_V210 || csp & X264_CSP_OTHER;
}
...
int x264_cli_pic_alloc(cli_pic_t *pic, int csp, int width, int height) { return cli_pic_init_internal(pic, csp, width, height, 1, 1); }
...
int x264_cli_pic_init_noalloc(cli_pic_t *pic, int csp, int width, int height) { return cli_pic_init_internal(pic, csp, width, height, 1, 0); }
